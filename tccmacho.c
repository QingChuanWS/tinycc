...
static void tcc_macho_add_destructor(TCCState* s1) {
  ...
  mh_execute_header = put_elf_sym(s1->symtab, -4096, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
                                  text_section->sh_num, "__mh_execute_header");
  s = find_section(s1, ".fini_array");
  if (s->data_offset == 0) return;
  init_sym =
      put_elf_sym(s1->symtab, text_section->data_offset, 0, ELFW(ST_INFO)(STB_LOCAL, STT_FUNC), 0,
                  text_section->sh_num, "___GLOBAL_init_65535");
  at_exit_sym = put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_FUNC), 0, SHN_UNDEF,
                            "___cxa_atexit");
  ...
  for_each_elem(s->reloc, 0, rel, ElfW_Rel) {
    ...
    ptr[1] = 0x8d;
    ptr[2] = 0x05;
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 23, R_X86_64_PC32,
                   sym_index, -4);
    ...
    ptr[15] = 0x8d;
    ptr[16] = 0x15;
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 9, R_X86_64_PC32,
                   mh_execute_header, -4);
    ptr[21] = 0xe8;  // call __cxa_atexit
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 4, R_X86_64_PLT32,
                   at_exit_sym, -4);
  }
  ...
  for_each_elem(s->reloc, 0, rel, ElfW_Rel) {
    ...
    put_elf_reloc(s1->symtab, text_section, text_section->data_offset - 24,
                  R_AARCH64_ADR_PREL_PG_HI21, sym_index);
    write32le(ptr, 0x90000000);  // adrp x0, destructor@page
    put_elf_reloc(s1->symtab, text_section, text_section->data_offset - 20,
...
#ifdef CONFIG_NEW_MACHO
static void bind_rebase_add(struct macho* mo, int bind, int sh_info, ElfW_Rel* rel,
                            struct sym_attr* attr) {
  mo->bind_rebase =
      tcc_realloc(mo->bind_rebase, (mo->n_bind_rebase + 1) * sizeof(struct bind_rebase));
  ...
  if (attr) mo->bind_rebase[mo->n_bind_rebase].rel.r_offset = attr->got_offset;
  mo->n_bind_rebase++;
  mo->n_bind += bind;
}
...
static void check_relocs(TCCState* s1, struct macho* mo) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    s = s1->sections[i];
    if (s->sh_type != SHT_RELX || !strncmp(s1->sections[s->sh_info]->name, ".debug_", 7)) continue;
    for_each_elem(s, 0, rel, ElfW_Rel) {
      ...
      if (sym->st_shndx == SHN_UNDEF || gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
        ...
        if (!attr->dyn_index) {
          ...
          put_elf_reloc(s1->symtab, s1->got, attr->got_offset, R_JMP_SLOT, sym_index);
        ...
        if (for_code && sym->st_shndx == SHN_UNDEF) {
          ...
          if (attr->plt_offset == -1) {
            ...
            jmp[1] = 0x25;
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset + 2, R_X86_64_GOTPCREL,
                          sym_index);
#elif defined TCC_TARGET_ARM64
            if (type != R_AARCH64_CALL26) continue;
            jmp = section_ptr_add(mo->stubs, 12);
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset, R_AARCH64_ADR_GOT_PAGE,
                          sym_index);
            write32le(jmp,  // adrp x16, #sym
                      0x90000010);
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset + 4, R_AARCH64_LD64_GOT_LO12_NC,
                          sym_index);
      ...
      if (type == R_DATA_PTR || type == R_JMP_SLOT)
        bind_rebase_add(mo, sym->st_shndx == SHN_UNDEF ? 1 : 0, s->sh_info, &save_rel, NULL);
    }
  }
...
static void check_relocs(TCCState* s1, struct macho* mo) {
  ...
  jmp[1] = 0x8d;
  jmp[2] = 0x1d;
  put_elf_reloca(s1->symtab, mo->stub_helper, 3, R_X86_64_PC32, mo->dyld_private, -4);
  jmp[7] = 0x41; /* pushq %r11 */
  jmp[8] = 0x53;
  jmp[9] = 0xff; /* jmpq    *dyld_stub_binder@GOT(%rip) */
  jmp[10] = 0x25;
  put_elf_reloca(s1->symtab, mo->stub_helper, 11, R_X86_64_GOTPCREL, mo->dyld_stub_binder, -4);
  jmp[15] = 0x90; /* nop */
#elif defined TCC_TARGET_ARM64
  jmp = section_ptr_add(mo->stub_helper, 24);
  put_elf_reloc(s1->symtab, mo->stub_helper, 0, R_AARCH64_ADR_PREL_PG_HI21, mo->dyld_private);
  write32le(jmp, 0x90000011);  // adrp x17, _dyld_private@page
  put_elf_reloc(s1->symtab, mo->stub_helper, 4, R_AARCH64_LDST64_ABS_LO12_NC, mo->dyld_private);
  write32le(jmp + 4, 0x91000231);  // add x17,x17,_dyld_private@pageoff
  write32le(jmp + 8, 0xa9bf47f0);  // stp x16/x17, [sp, #-16]!
  put_elf_reloc(s1->symtab, mo->stub_helper, 12, R_AARCH64_ADR_GOT_PAGE, mo->dyld_stub_binder);
  write32le(jmp + 12, 0x90000010);  // adrp x16, dyld_stub_binder@page
  put_elf_reloc(s1->symtab, mo->stub_helper, 16, R_AARCH64_LD64_GOT_LO12_NC, mo->dyld_stub_binder);
    ...
    for_each_elem(s, 0, rel, ElfW_Rel) {
      ...
      if (sym->st_shndx == SHN_UNDEF || gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
        ...
        if (!attr->dyn_index) {
          ...
          if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
            ...
            if (sym->st_shndx == SHN_UNDEF
              ...
              mo->bind = tcc_realloc(mo->bind, (mo->n_bind + 1) * sizeof(struct bind));
        ...
        if (for_code && sym->st_shndx == SHN_UNDEF) {
          if (attr->plt_offset == -1) {
            ...
            jmp[1] = 0x25;
            put_elf_reloca(s1->symtab, mo->stubs, mo->stubs->data_offset - 4, R_X86_64_PC32,
                           mo->lasym, mo->la_symbol_ptr->data_offset - 4);
            ...
            put_elf_reloca(s1->symtab, mo->la_symbol_ptr, mo->la_symbol_ptr->data_offset,
                           R_DATA_PTR, mo->helpsym, mo->stub_helper->data_offset - 10);
            section_ptr_add(mo->la_symbol_ptr, PTR_SIZE);
#elif defined TCC_TARGET_ARM64
            if (type != R_AARCH64_CALL26) continue;
            /* __stubs */
            jmp = section_ptr_add(mo->stubs, 12);
            put_elf_reloca(s1->symtab, mo->stubs, mo->stubs->data_offset - 12,
                           R_AARCH64_ADR_PREL_PG_HI21, mo->lasym, mo->la_symbol_ptr->data_offset);
            write32le(jmp,  // adrp x16, __la_symbol_ptr@page
                      0x90000010);
            put_elf_reloca(s1->symtab, mo->stubs, mo->stubs->data_offset - 8,
                           R_AARCH64_LDST64_ABS_LO12_NC, mo->lasym, mo->la_symbol_ptr->data_offset);
            ...
            write32le(jmp + 4,  // b stubHelperHeader
                      0x14000000 + ((-(mo->stub_helper->data_offset - 8) / 4) & 0x3ffffff));
            write32le(jmp + 8, 0);  // l0: .long bind_offset

            /* __la_symbol_ptr */
            la_symbol_offset = mo->la_symbol_ptr->data_offset;
            put_elf_reloca(s1->symtab, mo->la_symbol_ptr, mo->la_symbol_ptr->data_offset,
                           R_DATA_PTR, mo->helpsym, mo->stub_helper->data_offset - 12);
            section_ptr_add(mo->la_symbol_ptr, PTR_SIZE);
#endif
            mo->s_lazy_bind =
                tcc_realloc(mo->s_lazy_bind, (mo->n_lazy_bind + 1) * sizeof(struct s_lazy_bind));
            mo->s_lazy_bind[mo->n_lazy_bind].section = mo->stub_helper->reloc->sh_info;
            mo->s_lazy_bind[mo->n_lazy_bind].bind_offset = bind_offset;
            mo->s_lazy_bind[mo->n_lazy_bind].la_symbol_offset = la_symbol_offset;
            mo->s_lazy_bind[mo->n_lazy_bind].rel = save_rel;
            mo->s_lazy_bind[mo->n_lazy_bind].rel.r_offset = attr->plt_offset;
            mo->n_lazy_bind++;
            pi = section_ptr_add(mo->indirsyms, sizeof(*pi));
            *pi = mo->e2msym[sym_index];
            mo->nr_plt++;
      ...
      if (type == R_DATA_PTR || type == R_JMP_SLOT) {
        if (sym->st_shndx == SHN_UNDEF) {
          ...
          mo->s_rebase = tcc_realloc(mo->s_rebase, (mo->n_rebase + 1) * sizeof(struct s_rebase));
...
static int check_symbols(TCCState* s1, struct macho* mo) {
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
    ...
    if (bind == STB_LOCAL) {
      if (mo->ilocal == -1) mo->ilocal = sym_index - 1;
      if (mo->iextdef != -1 || mo->iundef != -1) tcc_error("local syms after global ones");
    } else if (sym->st_shndx != SHN_UNDEF) {
      if (mo->iextdef == -1) mo->iextdef = sym_index - 1;
      if (mo->iundef != -1) tcc_error("external defined symbol after undefined");
    } else if (sym->st_shndx == SHN_UNDEF) {
      if (mo->iundef == -1) mo->iundef = sym_index - 1;
      if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK || s1->output_type != TCC_OUTPUT_EXE ||
...
static void convert_symbol(TCCState* s1, struct macho* mo, struct nlist_64* pn) {
  ...
  switch (ELFW(ST_TYPE)(sym->st_info)) {
    ...
    case STT_FILE:
      ...
      break;
    default:
      tcc_error("unhandled ELF symbol type %d %s", ELFW(ST_TYPE)(sym->st_info), name);
  }
  if (sym->st_shndx == SHN_UNDEF)
    ...
    if (strncmp(s1->sections[sym->st_shndx]->name, ".debug_", 7))
      tcc_error("ELF section %d(%s) not mapped into Mach-O for symbol %s", sym->st_shndx,
                s1->sections[sym->st_shndx]->name, name);
  } else
...
static void create_symtab(TCCState* s1, struct macho* mo) {
  ...
  mo->stubsym = put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
                            mo->stubs->sh_num, ".__stubs");
#ifdef CONFIG_NEW_MACHO
  mo->chained_fixups = new_section(s1, "CHAINED_FIXUPS", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
#else
  ...
  mo->helpsym = put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
                            mo->stub_helper->sh_num, ".__stub_helper");
  mo->lasym = put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
                          mo->la_symbol_ptr->sh_num, ".__la_symbol_ptr");
  section_ptr_add(data_section, -data_section->data_offset & (PTR_SIZE - 1));
  mo->dyld_private =
      put_elf_sym(s1->symtab, data_section->data_offset, PTR_SIZE,
                  ELFW(ST_INFO)(STB_LOCAL, STT_OBJECT), 0, data_section->sh_num, ".__dyld_private");
  section_ptr_add(data_section, PTR_SIZE);
  mo->dyld_stub_binder = put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
...
const struct {
  ...
  const char* name;
    ...
    /*[sk_unknown] =*/{0},
    /*[sk_discard] =*/{0},
    /*[sk_text] =*/{1, S_REGULAR | S_ATTR_PURE_INSTRUCTIONS | S_ATTR_SOME_INSTRUCTIONS, "__text"},
    /*[sk_stubs] =*/
    {1, S_REGULAR | S_ATTR_PURE_INSTRUCTIONS | S_SYMBOL_STUBS | S_ATTR_SOME_INSTRUCTIONS,
     "__stubs"},
    /*[sk_stub_helper] =*/
...
static void calc_fixup_size(TCCState* s1, struct macho* mo) {
  ...
  size +=
      (sizeof(struct dyld_chained_starts_in_image) + (mo->nseg - 1) * sizeof(uint32_t) + 7) & -8;
  for (i = (s1->output_type == TCC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
    int page_count = (get_segment(mo, i)->vmsize + SEG_PAGE_SIZE - 1) / SEG_PAGE_SIZE;
    size +=
        (sizeof(struct dyld_chained_starts_in_segment) + (page_count - 1) * sizeof(uint16_t) + 7) &
        -8;
  }
...
static void set_segment_and_offset(TCCState* s1, struct macho* mo, addr_t addr, uint8_t* ptr,
                                   int opcode, Section* sec, addr_t offset) {
  int i;
  struct segment_command_64* seg = NULL;

  for (i = (s1->output_type == TCC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
    seg = get_segment(mo, i);
    if (addr >= seg->vmaddr && addr < (seg->vmaddr + seg->vmsize)) break;
  }
  *ptr = opcode | i;
...
static void bind_rebase(TCCState* s1, struct macho* mo) {
  ...
  for (i = 0; i < mo->n_lazy_bind; i++) {
    ...
    write32le(mo->stub_helper->data + mo->s_lazy_bind[i].bind_offset,
    ...
    set_segment_and_offset(s1, mo, mo->la_symbol_ptr->sh_addr, ptr,
                           BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB, mo->lazy_binding,
                           mo->s_lazy_bind[i].la_symbol_offset + mo->la_symbol_ptr->sh_addr);
    ptr = section_ptr_add(mo->lazy_binding, 5 + strlen(name));
    *ptr++ = BIND_OPCODE_SET_DYLIB_SPECIAL_IMM | (BIND_SPECIAL_DYLIB_FLAT_LOOKUP & 0xf);
  ...
  for (i = 0; i < mo->n_rebase; i++) {
    ...
    *ptr++ = REBASE_OPCODE_SET_TYPE_IMM | REBASE_TYPE_POINTER;
    set_segment_and_offset(s1, mo, s->sh_addr, ptr, REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB,
                           mo->rebase, mo->s_rebase[i].rel.r_offset + s->sh_addr);
  ...
  for (i = 0; i < mo->n_bind; i++) {
    ...
    sym = &((ElfW(Sym)*)symtab_section->data)[sym_index];
    name = (char*)symtab_section->link->data + sym->st_name;
    binding = ELFW(ST_BIND)(sym->st_info) == STB_WEAK ? mo->weak_binding : mo->binding;
    ptr = section_ptr_add(binding, 4 + (binding == mo->binding) + strlen(name));
    if (binding == mo->binding)
      *ptr++ = BIND_OPCODE_SET_DYLIB_SPECIAL_IMM | (BIND_SPECIAL_DYLIB_FLAT_LOOKUP & 0xf);
    *ptr++ = BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM |
             (binding == mo->weak_binding ? BIND_SYMBOL_FLAGS_WEAK_IMPORT : 0);
    strcpy((char*)ptr, name);
    ptr += strlen(name) + 1;
    *ptr++ = BIND_OPCODE_SET_TYPE_IMM | BIND_TYPE_POINTER;
    set_segment_and_offset(s1, mo, s->sh_addr, ptr, BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB,
                           binding, mo->bind[i].rel.r_offset + s->sh_addr);
...
static void create_trie(struct trie_node* node, int from, int to, int index_start, int n_trie,
                        struct trie_info* trie) {
  int i;
  ...
  for (i = from; i < to; i = end) {
    ...
    start = i++;
    for (; i < to; i++)
      if (cur != trie[i].name[index_start]) break;
    end = i;
    if (start == end - 1 ||
        (trie[start].name[index_start] && trie[start].name[index_start + 1] == 0))
      ...
      for (;;) {
        ...
        for (i = start + 1; i < end; i++)
          if (cur != trie[i].name[index_end]) break;
        if (trie[start].name[index_end] && trie[start].name[index_end + 1] == 0) {
          end = start + 1;
          index_end = trie[start].str_size - 1;
          break;
        }
    ...
    node->child = tcc_realloc(node->child, (node->n_child + 1) * sizeof(struct trie_node));
    ...
    child->child = NULL;
    node->n_child++;
    if (start != end - 1) create_trie(child, start, end, index_end, n_trie, trie);
  }
}

static int create_seq(int* offset, int* n_seq, struct trie_seq** seq, struct trie_node* node,
  ...
  for (i = 0; i < node->n_child; i++) {
    ...
    *seq = tcc_realloc(*seq, (*n_seq + 1) * sizeof(struct trie_seq));
    ...
    p_seq->offset = *offset;
    p_seq->nest_offset = 0;
    *offset += (i == 0 ? 1 + 1 : 0) + p_nest->index_end - p_nest->index_start + 1 + 3;
  }
  for (i = 0; i < node->n_child; i++) {
    nest_offset = create_seq(offset, n_seq, seq, &node->child[i], n_trie, trie);
...
static void node_free(struct trie_node* node) {
  ...
  for (i = 0; i < node->n_child; i++) node_free(&node->child[i]);
  tcc_free(node->child);
}
...
static void export_trie(TCCState* s1, struct macho* mo) {
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
    ...
    if (sym->st_shndx != SHN_UNDEF && sym->st_shndx < SHN_LORESERVE &&
      ...
      addr_t addr = sym->st_value + s1->sections[sym->st_shndx]->sh_addr - vm_addr;

      if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK) flag |= EXPORT_SYMBOL_FLAGS_WEAK_DEFINITION;
...
static void collect_sections(TCCState* s1, struct macho* mo, const char* filename) {
  ...
  for (i = 0; i < N_SEGMENT; i++) used_segment[i] = all_segment[i].used;
  ...
  if (s1->output_type != TCC_OUTPUT_EXE) used_segment[0] = 0;
  ...
  for (i = 0; i < N_SEGMENT; i++)
    if (used_segment[i]) {
      ...
      for (sk = sk_unknown; sk < sk_last; sk++)
        if (skinfo[sk].seg_initial == i) mo->segment[sk] = mo->nseg - 1;
    }
  ...
  if (s1->output_type != TCC_OUTPUT_EXE) {
    ...
    dylib->timestamp = 1;
    dylib->current_version = s1->current_version ? s1->current_version : 1 << 16;
    dylib->compatibility_version = s1->compatibility_version ? s1->compatibility_version : 1 << 16;
    str = (char*)dylib + dylib->name;
    strcpy(str, name);
  }

#ifdef CONFIG_NEW_MACHO
  chained_fixups_lc = add_lc(mo, LC_DYLD_CHAINED_FIXUPS, sizeof(struct linkedit_data_command));
  export_trie_lc = add_lc(mo, LC_DYLD_EXPORTS_TRIE, sizeof(struct linkedit_data_command));
#else
  ...
  for (i = 0; i < s1->nb_loaded_dlls; i++) {
    ...
    if (dllref->level == 0) add_dylib(mo, dllref->name);
  }
...
#endif
    if (skinfo[sk].seg_initial && (s1->output_type != TCC_OUTPUT_EXE || mo->segment[sk]) &&
        mo->sk_to_sect[sk].s) {
      uint64_t al = 0;
      int si;
      ...
      if (skinfo[sk].name) {
        ...
        if (sk == sk_stubs)
          ...
          sec->reserved2 = 12;
#endif
        if (sk == sk_nl_ptr) sec->reserved1 = mo->nr_plt;
#ifndef CONFIG_NEW_MACHO
        if (sk == sk_la_ptr) sec->reserved1 = mo->nr_plt + mo->n_got;
#endif
      }
      if (seg->vmaddr == -1) {
      ...
      if (sec) sec->align = al;
      al = 1ULL << al;
      if (al > 4096) tcc_warning("alignment > 4096"), sec->align = 12, al = 4096;
      ...
      for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
        ...
        if (sec) mo->elfsectomacho[s->sh_num] = numsec;
      }
      if (sec) sec->size = curaddr - sec->addr;
    }
    if (DEBUG_MACHO)
      for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
        int type = s->sh_type;
        ...
        printf("%d section %-16s %-10s %09lx %04x %02d %s,%s,%s\n", sk, s->name,
               ...
               : type == SHT_RELX       ? "rel"
                                        : "???",
               (long)s->sh_addr, (unsigned)s->data_offset, s->sh_addralign,
...
static void macho_write(TCCState* s1, struct macho* mo, FILE* fp) {
  ...
  for (i = 0; i < mo->nlc; i++) mo->mh.mh.sizeofcmds += mo->lc[i]->cmdsize;
  ...
  for (sk = sk_unknown; sk < sk_last; sk++) {
    // struct segment_command_64 *seg;
    if (skinfo[sk].seg_initial == 0 || (s1->output_type == TCC_OUTPUT_EXE && !mo->segment[sk]) ||
        !mo->sk_to_sect[sk].s)
      continue;
    /*seg =*/get_segment(mo, mo->segment[sk]);
    for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
      if (s->sh_type != SHT_NOBITS) {
        while (fileofs < s->sh_offset) fputc(0, fp), fileofs++;
        if (s->sh_size) {
...
ST_FUNC void bind_rebase_import(TCCState* s1, struct macho* mo) {
  ...
  tcc_qsort(mo->bind_rebase, mo->n_bind_rebase, sizeof(struct bind_rebase), bind_rebase_cmp, s1);
  for (i = 0; i < mo->n_bind_rebase - 1; i++)
    if (mo->bind_rebase[i].section == mo->bind_rebase[i + 1].section &&
      ...
      sym = &((ElfW(Sym)*)symtab_section->data)[sym_index];
      name = (char*)symtab_section->link->data + sym->st_name;
      tcc_error("Overlap %s/%s %s:%s", mo->bind_rebase[i].bind ? "bind" : "rebase",
  ...
  size = sizeof(struct dyld_chained_starts_in_image) + (mo->nseg - 1) * sizeof(uint32_t);
  image = (struct dyld_chained_starts_in_image*)data;
  data += (size + 7) & -8;
  image->seg_count = mo->nseg;
  for (i = (s1->output_type == TCC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
    image->seg_info_offset[i] = (data - mo->chained_fixups->data) - header->starts_offset;
    seg = get_segment(mo, i);
    page_count = (seg->vmsize + SEG_PAGE_SIZE - 1) / SEG_PAGE_SIZE;
    size = sizeof(struct dyld_chained_starts_in_segment) + (page_count - 1) * sizeof(uint16_t);
...
#define PTR_64_MASK 0xFFFFFFFFFFFFFFULL
    ...
    for (j = 0; j < page_count; j++) {
      ...
      for (; k < mo->n_bind_rebase; k++) {
        ...
        if ((addr & 3) || (addr & (SEG_PAGE_SIZE - 1)) > SEG_PAGE_SIZE - PTR_SIZE)
          tcc_error("Illegal rel_offset %s %lld", s->name, (long long)r_offset);
        if (addr >= end) break;
        if (addr >= start) {
          cur_o = addr - start;
          if (mo->bind_rebase[k].bind) {
            ...
            last = rebase;
            last_o = cur_o;
            cur = (*(uint64_t*)(s->data + r_offset)) - PTR_64_OFFSET;
  ...
  for (i = 0, bind_index = 0; i < mo->n_bind_rebase; i++) {
    if (mo->bind_rebase[i].bind) {
      import[bind_index].lib_ordinal = BIND_SPECIAL_DYLIB_FLAT_LOOKUP & 0xffu;
      import[bind_index].name_offset = (data - mo->chained_fixups->data) - header->symbols_offset;
      sym_index = ELFW(R_SYM)(mo->bind_rebase[i].rel.r_info);
      sym = &((ElfW(Sym)*)symtab_section->data)[sym_index];
      import[bind_index].weak_import = ELFW(ST_BIND)(sym->st_info) == STB_WEAK;
      name = (char*)symtab_section->link->data + sym->st_name;
...
do_ret:
  for (i = 0; i < mo.nlc; i++) tcc_free(mo.lc[i]);
  tcc_free(mo.seg2lc);
  tcc_free(mo.lc);
...
#define tbd_parse_tramplequote \
  if (*pos == '\'' || *pos == '"') tbd_parse_trample
#define tbd_parse_tramplespace \
  if (*pos == ' ') tbd_parse_trample
...
ST_FUNC void tcc_add_macos_sdkpath(TCCState* s) {
  ...
  cstr_new(&path);
  if (f) f(1, &sdkroot);
  if (sdkroot) pos = strstr(sdkroot, "SDKs/MacOSX");
  if (pos) cstr_printf(&path, "%.*s.sdk/usr/lib", (int)(pos - sdkroot + 11), sdkroot);
...
ST_FUNC int macho_load_tbd(TCCState* s1, int fd, const char* filename, int lev) {
  ...
  tbd_parse_trample;
  ret = 0;
  if (tcc_add_dllref(s1, soname, lev)->found) goto the_end;
  while (pos) {
    char* sym = NULL;
    int cont = 1;
    ...
    while (cont) {
      ...
      if (*pos == 0 || *pos == ']') cont = 0;
      tbd_parse_trample;
      set_elf_sym(s1->dynsymtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_UNDEF,
                  sym);
    }
  }
...
again:
  if (full_read(fd, buf, sizeof(buf)) != sizeof(buf)) return -1;
  ...
  if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
    ...
    tcc_warning("%s: Mach-O fat 64bit files of type 0x%x not handled", filename, fh.magic);
    return -1;
  }
  ...
  if (!nsyms || !nextdef) tcc_warning("%s doesn't export any symbols?", filename);
  ...
  for (i = iextdef; i < iextdef + nextdef; i++) {
    ...
    dprintf("%5d: %3d %3d 0x%04x 0x%016lx %s\n", i, sym->n_type, sym->n_sect, sym->n_desc,
            (long)sym->n_value, strtab + sym->n_strx);
    set_elf_sym(s1->dynsymtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_UNDEF,
                strtab + sym->n_strx);
  }
