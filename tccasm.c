...
static int asm2cname(int v, int* addeddot) {
  const char* name;
  *addeddot = 0;
  if (!tcc_state->leading_underscore) return v;
...
static Sym* asm_label_find(int v) {
  ...
  sym = sym_find(v);
  while (sym && sym->sym_scope && !(sym->type.t & VT_STATIC)) sym = sym->prev_tok;
  return sym;
}
...
static void asm_expr_unary(TCCState* s1, ExprValue* pe) {
  ...
  switch (tok) {
    case TOK_PPNUM:
      ...
      if (*p == 'b' || *p == 'f') {
        ...
        if (*p == 'b') {
          ...
          if (sym && (!sym->c || elfsym(sym)->st_shndx == SHN_UNDEF)) sym = sym->prev_tok;
          if (!sym) tcc_error("local label '%d' not found backward", (int)n);
        } else {
          /* forward */
    ...
    case '~':
      ...
      next();
      asm_expr_unary(s1, pe);
      if (pe->sym) tcc_error("invalid operation with label");
      if (op == '-')
        pe->v = -pe->v;
      else
        pe->v = ~pe->v;
...
static void asm_expr_prod(TCCState* s1, ExprValue* pe) {
  ...
  for (;;) {
    ...
    if (pe->sym || e2.sym) tcc_error("invalid operation with label");
    switch (op) {
      case '*':
        pe->v *= e2.v;
        break;
...
static inline void asm_expr_cmp(TCCState* s1, ExprValue* pe) {
  ...
  for (;;) {
    op = tok;
    if (op != TOK_EQ && op != TOK_NE && (op > TOK_GT || op < TOK_ULE)) break;
    next();
    asm_expr_sum(s1, &e2);
    if (pe->sym || e2.sym) tcc_error("invalid operation with label");
    switch (op) {
...
static Sym* asm_new_label1(TCCState* s1, int label, int is_local, int sh_num, int value) {
  ...
  if (sym) {
    ...
    if (esym && esym->st_shndx != SHN_UNDEF) {
      ...
      if (IS_ASM_SYM(sym) && (is_local == 1 || (sym->type.t & VT_EXTERN))) goto new_label;
  ...
  if (!sym->c) put_extern_sym2(sym, SHN_UNDEF, 0, 0, 1);
  esym = elfsym(sym);
  esym->st_shndx = sh_num;
  esym->st_value = value;
  if (is_local != 2) sym->type.t &= ~VT_EXTERN;
  return sym;
}
...
static void pop_section(TCCState* s1) {
  ...
  if (!prev) tcc_error(".popsection without .pushsection");
  ...
  switch (tok) {
    ...
    case TOK_ASMDIR_space:
      ...
      if (tok1 == TOK_ASMDIR_p2align) {
        if (n < 0 || n > 30) tcc_error("invalid p2align, must be between 0 and 30");
        n = 1 << n;
    ...
    case TOK_ASMDIR_org: {
      ...
      esym = elfsym(e.sym);
      if (esym) {
        if (esym->st_shndx != cur_text_section->sh_num) expect("constant or same-section symbol");
        n += esym->st_value;
      }
      if (n < ind) tcc_error("attempt to .org backwards");
      v = 0;
      size = n - ind;
      goto zero_pad;
    } break;
    ...
    case TOK_ASMDIR_hidden:
      ...
      do {
        ...
        next();
        sym = get_asm_sym(tok, NULL);
        if (tok1 != TOK_ASMDIR_hidden) sym->type.t &= ~VT_STATIC;
    ...
    case TOK_ASMDIR_asciz: {
      ...
      for (;;) {
        ...
        p = tokc.str.data;
        size = tokc.str.size;
        if (t == TOK_ASMDIR_ascii && size > 0) size--;
    ...
    case TOK_ASMDIR_section: {
      ...
      if (old_nb_section != s1->nb_sections) cur_text_section->sh_addralign = 1;
    } break;
    case TOK_ASMDIR_previous: {
      Section* sec;
      next();
      if (!last_text_section) tcc_error("no previous section referenced");
      sec = cur_text_section;
...
static int tcc_assemble_internal(TCCState* s1, int do_preprocess, int global) {
  ...
  redo:
    ...
    /* end of line */
    if (tok != ';' && tok != TOK_LINEFEED) expect("end of line");
    parse_flags &= ~PARSE_FLAG_LINEFEED; /* XXX: suppress that hack */
  }
...
/* find a constraint by its number or id (gcc 3 extended
   ...
   constraint */
ST_FUNC int find_constraint(ASMOperand* operands, int nb_operands, const char* name,
                            const char** pp) {
  int index;
  TokenSym* ts;
  const char* p;
...
static void subst_asm_operands(ASMOperand* operands, int nb_operands, CString* out_str,
                               CString* in_str) {
  ...
  for (;;) {
    ...
    if (c == '%') {
      ...
      modifier = 0;
      if (*str == 'c' || *str == 'n' || *str == 'b' || *str == 'w' || *str == 'h' || *str == 'k' ||
          *str == 'q' || *str == 'l' ||
      ...
      if (index < 0) tcc_error("invalid operand reference after %%");
      op = &operands[index];
      if (modifier == 'l') {
        ...
        if (op->reg >= 0) {
          sv.r = op->reg;
          if ((op->vt->r & VT_VALMASK) == VT_LLOCAL && op->is_memory) sv.r |= VT_LVAL;
        }
...
static void parse_asm_operands(ASMOperand* operands, int* nb_operands_ptr, int is_output) {
  ...
  if (tok != ':') {
    ...
    for (;;) {
      CString astr;
      if (nb_operands >= MAX_ASM_OPERANDS) tcc_error("too many asm operands");
      ...
      gexpr();
      if (is_output) {
        if (!(vtop->type.t & VT_ARRAY)) test_lvalue();
      } else {
      ...
      if (tok != ')') {
        ...
        if (tok == ':') {
          ...
          for (;;) {
            Sym* csym;
            int asmname;
            if (nb_operands + nb_labels >= MAX_ASM_OPERANDS) tcc_error("too many asm operands");
            ...
            if (!csym) {
              ...
              if (csym->r == LABEL_DECLARED) csym->r = LABEL_FORWARD;
            }
            next();
            asmname = asm_get_prefix_name(tcc_state, "LG.", ++asmgoto_n);
            if (!csym->c) put_extern_sym2(csym, SHN_UNDEF, 0, 0, 1);
...
/********************************************************/
#else
ST_FUNC int tcc_assemble(TCCState* s1, int do_preprocess) { tcc_error("asm not supported"); }
