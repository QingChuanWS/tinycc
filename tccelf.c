...
ST_FUNC void tccelf_delete(TCCState* s1) {
  ...
  for (i = 0; i < s1->nb_priv_sections; i++) free_section(s1->priv_sections[i]);
...
ST_FUNC Section* new_symtab(TCCState* s1, const char* symtab_name, int sh_type, int sh_flags,
                            const char* strtab_name, const char* hash_name, int hash_sh_flags) {
...
ST_FUNC size_t section_add(Section* sec, addr_t size, int align) {
  ...
  offset1 = offset + size;
  if (sec->sh_type != SHT_NOBITS && offset1 > sec->data_allocated) section_realloc(sec, offset1);
...
static void section_reserve(Section* sec, unsigned long size) {
  if (size > sec->data_allocated) section_realloc(sec, size);
  if (size > sec->data_offset) sec->data_offset = size;
}
#endif
...
static void rebuild_hash(Section* s, unsigned int nb_buckets) {
  ...
  if (!nb_buckets) nb_buckets = ((int*)s->hash->data)[0];
...
ST_FUNC int put_elf_sym(Section* s, addr_t value, unsigned long size, int info, int other,
                        int shndx, const char* name) {
...
ST_FUNC addr_t get_sym_addr(TCCState* s1, const char* name, int err, int forc) {
  ...
  if (!sym_index || sym->st_shndx == SHN_UNDEF) {
    if (err) tcc_error("%s not defined", name);
    return (addr_t)-1;
  }
  return sym->st_value;
}
...
static void version_add(TCCState* s1) {
  ...
  for (sym_index = 1; sym_index < end_sym; ++sym_index) {
    int dllindex, verndx;
    sym = &((ElfW(Sym)*)symtab->data)[sym_index];
    if (sym->st_shndx != SHN_UNDEF) continue; /* defined symbol doesn't need library version */
    name = (char*)symtab->link->data + sym->st_name;
    dllindex = find_elf_sym(s1->dynsymtab_section, name);
    verndx = (dllindex && dllindex < nb_sym_to_version) ? sym_to_version[dllindex] : -1;
    if (verndx >= 0) {
      if (!sym_versions[verndx].out_index) sym_versions[verndx].out_index = nb_versions++;
  ...
  if (nb_versions > 2) {
    ...
    for (i = nb_sym_versions; i-- > 0;) {
      ...
      if (strcmp(sv->lib, "ld-linux.so.2")) tcc_add_dllref(s1, sv->lib, 0);
      ...
      do {
        ...
        if (prev >= 0) sv = &sym_versions[prev];
      } while (prev >= 0);
      vna->vna_next = 0;
      vn = (ElfW(Verneed)*)(verneed_section->data + vnofs);
...
ST_FUNC int set_elf_sym(Section* s, addr_t value, unsigned long size, int info, int other,
                        int shndx, const char* name) {
  TCCState* s1 = s->s1;
  ElfW(Sym) * esym;
...
/* put relocation */
ST_FUNC void put_elf_reloca(Section* symtab, Section* s, unsigned long offset, int type, int symbol,
                            addr_t addend) {
  TCCState* s1 = s->s1;
  char buf[256];
  Section* sr;
  ...
  rel->r_addend = addend;
#endif
  if (SHT_RELX != SHT_RELA && addend) tcc_error("non-zero addend on REL architecture");
}

ST_FUNC void put_elf_reloc(Section* symtab, Section* s, unsigned long offset, int type,
                           int symbol) {
...
static Section* create_gnu_hash(TCCState* s1) {
  ...
  for (i = 0; i < nb_syms; i++, p++) ndef += p->st_shndx != SHN_UNDEF;
  ...
  while (ndef >= bloom_size * (1 << (bloom_shift - 3))) bloom_size *= 2;
...
static void update_gnu_hash(TCCState* s1, Section* gnu_hash) {
  ...
  if (gnu_hash->data_offset !=
      4 * 4 + PTR_SIZE * bloom_size + nbuckets * 4 + (nb_syms - (q - new_syms)) * 4)
    tcc_error("gnu_hash size incorrect");

  /* find buckets */
  for (i = 0; i < nbuckets; i++) buck[i].first = -1;
  ...
  if (vs) {
    ...
    if (1 /*versym*/) {
      ...
      for (i = 0; i < nb_syms; i++) newver[old_to_new_syms[i]] = versym[i];
    ...
    if (sh_num == SHN_UNDEF) {
      ...
      if (do_resolve) {
        ...
        if (addr == NULL) {
          int i;
          for (i = 0; i < s1->nb_loaded_dlls; i++)
            if ((addr = dlsym(s1->loaded_dlls[i]->handle, name))) break;
        }
#endif
        if (addr) {
...
ST_FUNC void relocate_sections(TCCState* s1) {
  ...
  for (i = 1; i < s1->nb_sections; ++i) {
    sr = s1->sections[i];
    if (sr->sh_type != SHT_RELX) continue;
...
#endif
        if (s1->output_type != TCC_OUTPUT_DLL) break;
        if (get_sym_attr(s1, sym_index, 0)->dyn_index) count++;
        break;
      default:
        break;
    }
...
static int build_got(TCCState* s1) {
  ...
  return set_elf_sym(symtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
...
static struct sym_attr* put_got_entry(TCCState* s1, int dyn_reloc_type, int sym_index) {
  ...
  /* In case a function is both called and its address taken 2 GOT entries
     ...
     entry (PLTGOT).  */
  if (need_plt_entry ? attr->plt_offset : attr->got_offset) return attr;
  ...
  if (s1->dynsym) {
    if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
      ...
      if (0 == attr->dyn_index)
        attr->dyn_index = set_elf_sym(s1->dynsym, sym->st_value, sym->st_size, sym->st_info, 0,
                                      sym->st_shndx, name);
  ...
  if (need_plt_entry) {
    ...
    len = strlen(name);
    if (len > sizeof plt_name - 5) len = sizeof plt_name - 5;
...
redo:
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    for_each_elem(s, 0, rel, ElfW_Rel) {
      ...
      if (gotplt_entry == -1) tcc_error("Unknown relocation type for got: %d", type);
      sym_index = ELFW(R_SYM)(rel->r_info);
      sym = &((ElfW(Sym)*)symtab_section->data)[sym_index];
      ...
      if (gotplt_entry == AUTO_GOTPLT_ENTRY) {
        if (sym->st_shndx == SHN_UNDEF) {
          ElfW(Sym) * esym;
          int dynindex;
          if (!PCRELATIVE_DLLPLT && (s1->output_type & TCC_OUTPUT_DYN)) continue;
      ...
      if ((type == R_X86_64_PLT32 || type == R_X86_64_PC32) && sym->st_shndx != SHN_UNDEF &&
          (ELFW(ST_VISIBILITY)(sym->st_other) != STV_DEFAULT ||
           ELFW(ST_BIND)(sym->st_info) == STB_LOCAL || s1->output_type & TCC_OUTPUT_EXE)) {
        if (pass != 0) continue;
      ...
      if (reloc_type == -1) tcc_error("Unknown relocation type: %d", type);
      ...
      if (gotplt_entry == BUILD_GOT_ONLY) continue;

      attr = put_got_entry(s1, reloc_type, sym_index);

      if (reloc_type == R_JMP_SLOT) rel->r_info = ELFW(R_INFO)(attr->plt_sym, type);
    }
  }
  if (++pass < 2) goto redo;
  /* .rel.plt refers to .got actually */
  if (s1->plt && s1->plt->reloc) s1->plt->reloc->sh_info = s1->got->sh_num;
  if (got_sym) /* set size */
...
ST_FUNC int set_global_sym(TCCState* s1, const char* name, Section* sec, addr_t offs) {
  ...
  if (sec && offs == -1) offs = sec->data_offset;
...
#ifdef CONFIG_TCC_BACKTRACE
    if (s1->do_backtrace) {
      if (s1->output_type & TCC_OUTPUT_EXE) tcc_add_support(s1, "bt-exe.o");
      if (s1->output_type != TCC_OUTPUT_DLL) tcc_add_support(s1, "bt-log.o");
      if (s1->output_type != TCC_OUTPUT_MEMORY) tcc_add_btstub(s1);
    }
#endif
    if (lpthread) tcc_add_library_err(s1, "pthread");
...
#endif
    if (TCC_LIBTCC1[0]) tcc_add_support(s1, TCC_LIBTCC1);
...
static void tcc_add_linker_symbols(TCCState* s1) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    s = s1->sections[i];
    if ((s->sh_flags & SHF_ALLOC) && (s->sh_type == SHT_PROGBITS || s->sh_type == SHT_STRTAB)) {
      const char* p;
...
static void fill_local_got_entries(TCCState* s1) {
  ...
  for_each_elem(s1->got->reloc, 0, rel, ElfW_Rel) {
    if (ELFW(R_TYPE)(rel->r_info) == R_RELATIVE) {
      ...
      struct sym_attr* attr = get_sym_attr(s1, sym_index, 0);
      unsigned offset = attr->got_offset;
      if (offset != rel->r_offset - s1->got->sh_addr) tcc_error_noabort("huh");
    ...
    if (sym->st_shndx == SHN_UNDEF) {
      ...
      if (sym_index) {
        ...
        if ((type == STT_FUNC) || (type == STT_GNU_IFUNC)) {
          ...
          set_elf_sym(s1->symtab, offset, esym->st_size, esym->st_info, 0, bss_section->sh_num,
                      name);
          ...
          if (ELFW(ST_BIND)(esym->st_info) == STB_WEAK) {
            for_each_elem(s1->dynsymtab_section, 1, dynsym, ElfW(Sym)) {
              if ((dynsym->st_value == esym->st_value) &&
                ...
                put_elf_sym(s1->dynsym, offset, dynsym->st_size, dynsym->st_info, 0,
...
static void export_global_syms(TCCState* s1) {
  ...
  for_each_elem(symtab_section, 1, sym, ElfW(Sym)) {
    if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
      name = (char*)symtab_section->link->data + sym->st_name;
      dynindex = set_elf_sym(s1->dynsym, sym->st_value, sym->st_size, sym->st_info, 0,
                             sym->st_shndx, name);
    ...
    if (s->sh_type == SHT_RELX && !(s->sh_flags & SHF_ALLOC)) {
      ...
      if ((file_type & TCC_OUTPUT_DYN) && (s1->sections[s->sh_info]->sh_flags & SHF_ALLOC)) {
        ...
        if (count) {
          ...
          if (!(s1->sections[s->sh_info]->sh_flags & SHF_WRITE)) textrel += count;
        }
      }
...
static int sort_sections(TCCState* s1, int* sec_order, Section* interp) {
  ...
  for (i = 1; i < nb_sections; i++) {
    ...
    if (s->sh_flags & SHF_ALLOC) {
      j = 0x100;
      if (s->sh_flags & SHF_WRITE) j = 0x200;
      if (s->sh_flags & SHF_TLS) j += 0x200;
    } else if (s->sh_name) {
      j = 0x700;
    } else {
    ...
    if (s->sh_type == SHT_SYMTAB || s->sh_type == SHT_DYNSYM) {
      ...
      k = 0x20;
      if (s1->plt && s == s1->plt->reloc) k = 0x21;
      ...
      k = 0x50;
      if (s->sh_type == SHT_NOTE) k = 0x60;
      if (s->sh_flags & SHF_EXECINSTR) k = 0x70;
      if (s->sh_type == SHT_NOBITS) k = 0x80;
      if (s == interp) k = 0x00;
    }
    k += j;
...
static int layout_sections(TCCState* s1, int* sec_order, struct dyn_inf* d) {
  ...
  if (s1->section_align) s_align = s1->section_align;

  addr = ELF_START_ADDR;
  if (s1->output_type & TCC_OUTPUT_DYN) addr = 0;
  ...
  if (s1->has_text_addr) {
    ...
    if (0) {
      ...
      if (a_offset < p_offset) a_offset += s_align;
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if ((f & 1 << 8) && n) {
      ...
      if (s1->output_format == TCC_OUTPUT_FORMAT_ELF) {
        ...
        if ((addr & (s_align - 1)) != 0) addr += s_align;
      } else {
        align = s_align - 1;
      }
    }
    ...
    if (f & 1 << 8) {
      ...
      ph->p_align = s_align;
      ph->p_flags = PF_R;
      if (f & SHF_WRITE) ph->p_flags |= PF_W;
      if (f & SHF_EXECINSTR) ph->p_flags |= PF_X;
      if (f & SHF_TLS) {
        ph->p_type = PT_TLS;
        ph->p_align = 4;
      }
  ...
  /* Fill other headers */
  if (d->note) fill_phdr(++ph, PT_NOTE, d->note);
  if (d->dynamic) fill_phdr(++ph, PT_DYNAMIC, d->dynamic)->p_flags |= PF_W;
  if (d->roinf) fill_phdr(++ph, PT_GNU_RELRO, d->roinf)->p_flags |= PF_W;
  if (d->interp) fill_phdr(&d->phdr[1], PT_INTERP, d->interp);
  if (phfill) {
    ph = &d->phdr[0];
...
static void tcc_output_elf(TCCState* s1, FILE* f, int phnum, ElfW(Phdr) * phdr, int file_offset,
                           int* sec_order) {
  ...
  if (file_type == TCC_OUTPUT_OBJ) {
    ...
    if (ehdr.e_entry == (addr_t)-1) ehdr.e_entry = text_section->sh_addr;
  }
  ...
  if (phdr) fwrite(phdr, 1, phnum * sizeof(ElfW(Phdr)), f);
...
static int tidy_section_headers(TCCState* s1, int* sec_order) {
  ...
  for_each_elem(symtab_section, 1, sym,
                ElfW(Sym)) if (sym->st_shndx != SHN_UNDEF && sym->st_shndx < SHN_LORESERVE)
  ...
  for (i = 0; i < s1->nb_sections; i++) sec_order[i] = i;
...
static int elf_output_file(TCCState* s1, const char* filename) {
  ...
  if (!s1->static_link) {
    if (file_type & TCC_OUTPUT_EXE) {
      ...
      if (elfint == NULL) elfint = DEFAULT_ELFINTERP(s1);
  ...
  if (!s1->static_link) {
    ...
    if (file_type & TCC_OUTPUT_DYN) {
      ...
      if (file_type & TCC_OUTPUT_EXE) dt_flags_1 = DF_1_NOW | DF_1_PIE;
    }
    put_dt(dynamic, DT_FLAGS, DF_BIND_NOW);
    put_dt(dynamic, DT_FLAGS_1, dt_flags_1);
    if (s1->symbolic) put_dt(dynamic, DT_SYMBOLIC, 0);
  ...
  if (s1->nb_errors != 0) goto the_end;
  ...
  if (dyninf.gnu_hash) update_gnu_hash(s1, dyninf.gnu_hash);
...
static void alloc_sec_names(TCCState* s1, int is_obj) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    s = s1->sections[i];
    if (is_obj) s->sh_size = s->data_offset;
...
LIBTCCAPI int tcc_output_file(TCCState* s, const char* filename) {
  if (s->test_coverage) tcc_tcov_add_file(s, filename);
  if (s->output_type == TCC_OUTPUT_OBJ) return elf_output_obj(s, filename);
#ifdef TCC_TARGET_PE
...
ST_FUNC int tcc_object_type(int fd, ElfW(Ehdr) * h) {
  ...
  if (size == sizeof *h && 0 == memcmp(h, ELFMAG, 4)) {
    if (h->e_type == ET_REL) return AFF_BINTYPE_REL;
    if (h->e_type == ET_DYN) return AFF_BINTYPE_DYN;
  } else if (size >= 8) {
    if (0 == memcmp(h, ARMAG, 8)) return AFF_BINTYPE_AR;
#ifdef TCC_TARGET_COFF
    if (((struct filehdr*)h)->f_magic == COFF_C67_MAGIC) return AFF_BINTYPE_C67;
#endif
  }
  return 0;
}
...
ST_FUNC int tcc_load_object_file(TCCState* s1, int fd, unsigned long file_offset) {
  ...
  lseek(fd, file_offset, SEEK_SET);
  if (tcc_object_type(fd, &ehdr) != AFF_BINTYPE_REL) goto fail1;
  ...
  for (i = 1; i < ehdr.e_shnum; i++) {
    ...
    if (sh->sh_flags & SHF_COMPRESSED) seencompressed = 1;
  }
  ...
  for (i = 1; i < ehdr.e_shnum; i++) {
    ...
    sh = &shdr[i];
    if (sh->sh_type == SHT_RELX) sh = &shdr[sh->sh_info];
...
#endif
        sh->sh_type != SHT_NOTE && sh->sh_type != SHT_NOBITS && sh->sh_type != SHT_PREINIT_ARRAY &&
        sh->sh_type != SHT_INIT_ARRAY && sh->sh_type != SHT_FINI_ARRAY &&
        strcmp(strsec + sh->sh_name, ".stabstr"))
      continue;
    if (seencompressed && 0 == strncmp(strsec + sh->sh_name, ".debug_", 7)) continue;
    ...
    for (j = 1; j < s1->nb_sections; j++) {
      ...
      if (!strcmp(s->name, sh_name)) {
        ...
        if (stab_section) {
          if (s == stab_section) stab_index = i;
          if (s == stab_section->link) stabstr_index = i;
        }
        goto found;
      }
    }
    ...
    switch (s->sh_type) {
      case SHT_RELX:
        ...
        for (rel = (ElfW_Rel*)s->data + (offset / sizeof(*rel));
          ...
          invalid_reloc:
            tcc_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x", i, strsec + sh->sh_name,
                              (int)rel->r_offset);
            goto fail;
          }
          rel->r_info = ELFW(R_INFO)(sym_index, type);
          ...
          if (type == R_ARM_THM_JUMP24) get_sym_attr(s1, sym_index, 1)->plt_thumb_stub = 1;
#endif
        }
        break;
      default:
...
static int read_ar_header(int fd, int offset, ArchiveHeader* hdr) {
  ...
  if (len != sizeof(ArchiveHeader)) return len ? -1 : 0;
  p = hdr->ar_name;
  for (e = p + sizeof hdr->ar_name; e > p && e[-1] == ' ';) --e;
  *e = '\0';
...
static int tcc_load_alacarte(TCCState* s1, int fd, int size, int entrysize) {
  ...
  data = tcc_malloc(size);
  if (full_read(fd, data, size) != size) goto the_end;
  ...
  do {
    ...
    for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p) + 1) {
      ...
      if (tcc_load_object_file(s1, fd, off) < 0) goto the_end;
      ++bound;
    }
  } while (bound);
  ret = 0;
...
ST_FUNC int tcc_load_archive(TCCState* s1, int fd, int alacarte) {
  ...
  for (;;) {
    ...
    if (alacarte) {
      ...
      if (!strcmp(hdr.ar_name, "/")) return tcc_load_alacarte(s1, fd, size, 4);
      if (!strcmp(hdr.ar_name, "/SYM64/")) return tcc_load_alacarte(s1, fd, size, 8);
...
static void set_ver_to_ver(TCCState* s1, int* n, int** lv, int i, char* lib, char* version) {
  ...
  if ((*lv)[i] == -1) {
    ...
    for (v = 0; v < nb_sym_versions; v++) {
      if (strcmp(sym_versions[v].lib, lib)) continue;
      prev_same_lib = v;
      if (!strcmp(sym_versions[v].version, version)) break;
    }
...
static void set_sym_version(TCCState* s1, int sym_index, int verndx) {
  ...
  if (sym_to_version[sym_index] < 0) sym_to_version[sym_index] = verndx;
}
...
#if DEBUG_VERSION
      printf("verdef: version:%u flags:%u index:%u, hash:%u\n", vdef->vd_version, vdef->vd_flags,
             vdef->vd_ndx, vdef->vd_hash);
#endif
      if (vdef->vd_cnt) {
...
ST_FUNC int tcc_load_dll(TCCState* s1, int fd, const char* filename, int level) {
  ...
  if (!dynamic) goto the_end;

  /* compute the real library name */
  soname = tcc_basename(filename);
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
    if (dt->d_tag == DT_SONAME) soname = dynstr + dt->d_un.d_val;

  /* if the dll is already loaded, do not load it */
  if (tcc_add_dllref(s1, soname, level)->found) goto ret_success;
  ...
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
    if (dt->d_tag == DT_RPATH) tcc_add_library_path(s1, dynstr + dt->d_un.d_val);
  ...
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
    switch (dt->d_tag) {
      case DT_NEEDED:
        ...
        if (tcc_add_dllref(s1, name, -1)) continue;
...
static int ld_add_file_list(TCCState* s1, const char* cmd, int as_needed) {
  ...
  if (!as_needed) s1->new_undef_sym = 0;
    ...
    if (!strcmp(filename, "AS_NEEDED")) {
      ...
      if (!as_needed) {
        ...
        if (group) {
          ...
          if (libname[0] != '\0') dynarray_add(&libs, &nblibs, tcc_strdup(libname));
        }
      }
    }
    t = ld_next(s1, filename, sizeof(filename));
  ...
  if (group && !as_needed) {
    while (s1->new_undef_sym) {
      int i;
      s1->new_undef_sym = 0;
      for (i = 0; i < nblibs; i++) ld_add_file(s1, libs[i]);
    }
  }
lib_parse_error:
