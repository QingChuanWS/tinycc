...
static const unsigned char tok_two_chars[] =
    ...
    */
    {'<', '=', TOK_LE,        '>', '=', TOK_GE,     '!', '=', TOK_NE,    '&', '&', TOK_LAND,
     '|', '|', TOK_LOR,       '+', '+', TOK_INC,    '-', '-', TOK_DEC,   '=', '=', TOK_EQ,
     '<', '<', TOK_SHL,       '>', '>', TOK_SAR,    '+', '=', TOK_A_ADD, '-', '=', TOK_A_SUB,
     '*', '=', TOK_A_MUL,     '/', '=', TOK_A_DIV,  '%', '=', TOK_A_MOD, '&', '=', TOK_A_AND,
     '^', '=', TOK_A_XOR,     '|', '=', TOK_A_OR,   '-', '>', TOK_ARROW, '.', '.', TOK_TWODOTS,
     '#', '#', TOK_TWOSHARPS, '#', '#', TOK_PPJOIN, 0};

static void next_nomacro(void);

ST_FUNC void skip(int c) {
  if (tok != c) tcc_error("'%c' expected (got \"%s\")", c, get_tok_str(tok, &tokc));
  next();
}
...
tail_call:
  if (al->buffer <= (uint8_t*)p && (uint8_t*)p < al->buffer + al->size) {
    ...
    if (header->line_num < 0) {
      ...
      fprintf(stderr, "%s:%d: %d bytes\n", header->file_name, (int)-header->line_num,
              (int)header->size);
    } else
      header->line_num = -header->line_num;
#endif
    al->nb_allocs--;
    if (!al->nb_allocs) al->p = al->buffer;
  } else if (al->next) {
    al = al->next;
    goto tail_call;
  } else
...
#ifdef TAL_INFO
      if (al->nb_peak < al->nb_allocs) al->nb_peak = al->nb_allocs;
      if (al->peak_p < al->p) al->peak_p = al->p;
      al->nb_total++;
#endif
      return ret;
    } else if (is_own) {
...
static void cstr_realloc(CString* cstr, int new_size) {
  ...
  if (size < 8) size = 8; /* no need to allocate a too small first string */
...
ST_INLN char* unicode_to_utf8(char* b, uint32_t Uc) {
  if (Uc < 0x80)
    ...
    *b++ = 240 + Uc / 262144, *b++ = 128 + Uc / 4096 % 64, *b++ = 128 + Uc / 64 % 64,
    *b++ = 128 + Uc % 64;
  else
  error:
...
ST_FUNC int tok_alloc_const(const char* str) { return tok_alloc(str, strlen(str))->tok; }
...
ST_FUNC const char* get_tok_str(int v, CValue* cv) {
  ...
  switch (v) {
    ...
    case TOK_STR:
      ...
      if (v == TOK_STR) {
        len = cv->str.size - 1;
        for (i = 0; i < len; i++) add_char(&cstr_buf, ((unsigned char*)cv->str.data)[i]);
      } else {
        len = (cv->str.size / sizeof(nwchar_t)) - 1;
        for (i = 0; i < len; i++) add_char(&cstr_buf, ((nwchar_t*)cv->str.data)[i]);
      }
      cstr_ccat(&cstr_buf, '\"');
      cstr_ccat(&cstr_buf, '\0');
      break;
...
static int next_c(void) {
  ...
  if (ch == CH_EOB && file->buf_ptr >= file->buf_end) ch = handle_eob();
  return ch;
}
...
static int handle_stray_noerror(int err) {
  ...
  while ((ch = next_c()) == '\\') {
    ...
    newl:
      ...
      if (err) tcc_error("stray '\\' in program");
...
static uint8_t* parse_comment(uint8_t* p) {
  ...
  for (;;) {
    ...
    if (c == '\n') {
      ...
      c = handle_bs(&p);
    check_eof:
      if (c == CH_EOF) tcc_error("unexpected end of file in comment");
      if (c != '\\') goto redo;
    }
  }
  return p + 1;
  ...
  redo:
    if (c == sep) {
      ...
      unterminated_string:
        ...
        if (c == '\\') {
          c = handle_bs(&p);
          if (c == CH_EOF) goto unterminated_string;
        }
        goto add_char;
      } else {
        goto redo;
    ...
    switch (c) {
      ...
      case '#':
        ...
        if (start_of_line) {
          ...
          next_nomacro();
          p = file->buf_ptr;
          if (a == 0 && (tok == TOK_ELSE || tok == TOK_ELIF || tok == TOK_ENDIF)) goto the_end;
...
ST_FUNC void tok_str_add(TokenString* s, int t) {
  ...
  len = s->len;
  str = s->str;
  if (len >= s->allocated_len) str = tok_str_realloc(s, len + 1);
  str[len++] = t;
  s->len = len;
}
...
ST_FUNC void end_macro(void) {
  ...
  if (str->alloc != 0) {
    if (str->alloc == 2) str->str = NULL; /* don't free */
    tok_str_free(str);
  }
}
...
static void tok_str_add2(TokenString* s, int t, CValue* cv) {
  ...
  if (len + TOK_MAX_SIZE >= s->allocated_len) str = tok_str_realloc(s, len + TOK_MAX_SIZE + 1);
  str[len++] = t;
  switch (t) {
    case TOK_CINT:
    case TOK_CUINT:
    ...
    case TOK_LSTR: {
      ...
      if (len + nb_words >= s->allocated_len) str = tok_str_realloc(s, len + nb_words + 1);
      str[len] = cv->str.size;
...
ST_INLN void define_push(int v, int macro_type, int* str, Sym* first_arg) {
  ...
  if (o && !macro_is_equal(o->d, s->d)) tcc_warning("%s redefined", get_tok_str(v, NULL));
}

/* undefined a define symbol. Its name is just set to zero */
ST_FUNC void define_undef(Sym* s) {
  int v = s->v;
  if (v >= TOK_IDENT && v < tok_ident) table_ident[v - TOK_IDENT]->sym_define = NULL;
}
...
ST_FUNC void label_pop(Sym** ptop, Sym* slast, int keep) {
  ...
  for (s = *ptop; s != slast; s = s1) {
    ...
    /* remove label */
    if (s->r != LABEL_GONE) table_ident[s->v - TOK_IDENT]->sym_label = s->prev_tok;
    if (!keep)
      sym_free(s);
    else
      s->r = LABEL_GONE;
...
static void maybe_run_test(TCCState* s) {
  const char* p;
  if (s->include_stack_ptr != s->include_stack) return;
  p = get_tok_str(tok, NULL);
  if (0 != memcmp(p, "test_", 5)) return;
...
static int parse_include(TCCState* s1, int do_next, int test) {
  ...
  if (c == '<' || c == '\"') {
    ...
    for (;;) {
      ...
      if (i > 0 && ((p[0] == '"' && p[i] == '"') || (p[0] == '<' && p[i] == '>'))) break;
      if (tok == TOK_LINEFEED) tcc_error("'#include' expects \"FILENAME\" or <FILENAME>");
  ...
  if (test) {
    ...
    *s1->include_stack_ptr++ = file->prev;
    ...
    if (s1->gen_deps) {
      BufferedFile* bf = file;
      while (i == 1 && (bf = bf->prev)) i = bf->include_next_index;
...
static int expr_preprocess(TCCState* s1) {
  ...
  redo:
    if (tok == TOK_DEFINED) {
      ...
      if (s1->run_test) maybe_run_test(s1);
      c = 0;
      if (define_find(tok) || tok == TOK___HAS_INCLUDE || tok == TOK___HAS_INCLUDE_NEXT) c = 1;
      if (t == '(') {
        next_nomacro();
        if (tok != ')') expect("')'");
      }
      ...
      tok_str_add_tok(str);
      next();
      if (tok == '(') tcc_error("function-like macro '%s' is not defined", get_tok_str(t, NULL));
      goto redo;
    }
    tok_str_add_tok(str);
  }
...
ST_FUNC void parse_define(void) {
    ...
    if (tok != ')')
      for (;;) {
        ...
        *ps = s;
        ps = &s->next;
        if (tok == ')') break;
        if (tok != ',' || is_vaargs) goto bad_list;
        next_nomacro();
      }
    parse_flags |= PARSE_FLAG_SPACES;
    next_nomacro();
  ...
  while (tok != TOK_LINEFEED && tok != TOK_EOF) {
    ...
    if (TOK_TWOSHARPS == tok) {
      if (2 == spc) goto bad_twosharp;
      if (1 == spc) --tokstr_buf.len;
      spc = 3;
      tok = TOK_PPJOIN;
    } else if ('#' == tok) {
      spc = 4;
  ...
  if (spc == 1) --tokstr_buf.len; /* remove trailing space */
...
static CachedInclude* search_cached_include(TCCState* s1, const char* filename, int add) {
  ...
  for (;;) {
    ...
    if (0 == PATHCMP(e->filename, filename)) return e;
    i = e->hash_next;
  }
  if (!add) return NULL;
...
static void pragma_parse(TCCState* s1) {
  ...
  if (tok == TOK_push_macro || tok == TOK_pop_macro) {
    ...
    if (next(), tok != ')') goto pragma_err;
    if (t == TOK_push_macro) {
      while (NULL == (s = define_find(v))) define_push(v, 0, NULL, NULL);
    ...
    if (tok == TOK_ASM_pop) {
      ...
      if (tok != ')') {
        ...
        val = tokc.i;
        if (val < 1 || val > 16 || (val & (val - 1)) != 0) goto pragma_err;
        next();
      }
    pack_set:
    ...
    if (t == TOK_lib) {
      ...
      if (t == TOK_option) tcc_set_options(s1, p);
      tcc_free(p);
    }
...
ST_FUNC void preprocess(int is_bof) {
  ...
  parse_flags = PARSE_FLAG_PREPROCESS | PARSE_FLAG_TOK_NUM | PARSE_FLAG_TOK_STR |
                PARSE_FLAG_LINEFEED | (parse_flags & PARSE_FLAG_ASM_FILE);
...
redo:
  switch (tok) {
    ...
    do_ifdef:
      next_nomacro();
      if (tok < TOK_IDENT) tcc_error("invalid argument for '#if%sdef'", c ? "n" : "");
      if (is_bof) {
        if (c) {
#ifdef INC_DEBUG
    ...
    do_if:
      ...
      *s1->ifdef_stack_ptr++ = c;
      goto test_skip;
    case TOK_ELSE:
      if (s1->ifdef_stack_ptr == s1->ifdef_stack) tcc_error("#else without matching #if");
      if (s1->ifdef_stack_ptr[-1] & 2) tcc_error("#else after #else");
      c = (s1->ifdef_stack_ptr[-1] ^= 3);
      goto test_else;
    case TOK_ELIF:
      if (s1->ifdef_stack_ptr == s1->ifdef_stack) tcc_error("#elif without matching #if");
      c = s1->ifdef_stack_ptr[-1];
      if (c > 1) tcc_error("#elif after #else");
    ...
    test_else:
      if (s1->ifdef_stack_ptr == file->ifdef_stack_ptr + 1) file->ifndef_macro = 0;
    test_skip:
      if (!(c & 1)) {
        preprocess_skip();
        is_bof = 0;
      ...
      break;
    case TOK_ENDIF:
      if (s1->ifdef_stack_ptr <= file->ifdef_stack_ptr) tcc_error("#endif without matching #if");
      s1->ifdef_stack_ptr--;
    ...
    _line_num:
      ...
      if (file->fd > 0) total_lines += file->line_num - n;
      file->line_num = n;
      break;
    case TOK_ERROR:
    case TOK_WARNING:
    ...
    default:
      ...
      if (saved_parse_flags & PARSE_FLAG_ASM_FILE) goto ignore;
      if (tok == '!' && is_bof) /* '!' is ignored at beginning to allow C scripts. */
        goto ignore;
...
static void parse_string(const char* s, int len) {
  ...
  if (sep == '\'') {
    ...
    if (n > 1) tcc_warning_c(warn_all)("multi-character character constant");
    for (c = i = 0; i < n; ++i) {
      if (is_long)
  ...
  if (ch == '.' || ((ch == 'e' || ch == 'E') && b == 10) ||
    ...
    *q = '\0';
    ...
    for (;;) {
      t = toup(ch);
      if (t == 'L') {
        if (lcount >= 2) tcc_error("three 'l's in integer constant");
        if (lcount && *(p - 1) != ch) tcc_error("incorrect integer suffix: %s", p1);
        lcount++;
        ch = *p++;
      } else if (t == 'U') {
        if (ucount >= 1) tcc_error("two 'u's in integer constant");
        ucount++;
        ch = *p++;
      } else {
        break;
    ...
    if (ucount == 0 && b == 10) {
      if (lcount <= (LONG_SIZE == 4)) {
        if (n >= 0x80000000U) lcount = (LONG_SIZE == 4) + 1;
      }
      if (n >= 0x8000000000000000ULL) ov = 1, ucount = 1;
    } else {
      if (lcount <= (LONG_SIZE == 4)) {
        if (n >= 0x100000000ULL)
      ...
      if (n >= 0x8000000000000000ULL) ucount = 1;
    }

    if (ov) tcc_warning("integer constant overflow");
...
redo_no_start:
  ...
  switch (c) {
    ...
    case '\t':
      ...
      p++;
    maybe_space:
      if (parse_flags & PARSE_FLAG_SPACES) goto keep_tok_flags;
      while (isidnum_table[*p - CH_EOF] & IS_SPC) ++p;
      goto redo_no_start;
    case '\f':
    case '\v':
    case '\r':
    ...
    case '\\':
      ...
      if (c == CH_EOF) {
        ...
        if ((parse_flags & PARSE_FLAG_LINEFEED) && !(tok_flags & TOK_FLAG_EOF)) {
          ...
          p = file->buf_ptr;
          if (p == file->buffer) tok_flags = TOK_FLAG_BOF;
          tok_flags |= TOK_FLAG_BOL;
          goto redo_no_start;
        }
      } else {
    ...
    case '\n':
      ...
      p++;
    maybe_newline:
      if (0 == (parse_flags & PARSE_FLAG_LINEFEED)) goto redo_no_start;
      tok = TOK_LINEFEED;
      goto keep_tok_flags;
    ...
    parse_ident_fast:
      ...
      h = TOK_HASH_INIT;
      h = TOK_HASH_FUNC(h, c);
      while (c = *++p, isidnum_table[c - CH_EOF] & (IS_ID | IS_NUM)) h = TOK_HASH_FUNC(h, c);
      len = p - p1;
      if (c != '\\') {
        TokenSym** pts;
    ...
    str_const:
      cstr_reset(&tokcstr);
      if (is_long) cstr_ccat(&tokcstr, 'L');
      cstr_ccat(&tokcstr, c);
    ...
    default:
      if (c >= 0x80 && c <= 0xFF) /* utf8 identifiers */
        goto parse_ident_fast;
      if (parse_flags & PARSE_FLAG_ASM_FILE) goto parse_simple;
...
static int paste_tokens(int t1, CValue* v1, int t2, CValue* v2) {
  ...
  cstr_new(&cstr);
  if (t1 != TOK_PLCHLDR) cstr_cat(&cstr, get_tok_str(t1, v1), -1);
  n = cstr.size;
  if (t2 != TOK_PLCHLDR) cstr_cat(&cstr, get_tok_str(t2, v2), -1);
  cstr_ccat(&cstr, '\0');
  ...
  for (;;) {
    next_nomacro1();
    if (0 == *file->buf_ptr) break;
    if (is_space(tok)) continue;
    tcc_warning(
...
static inline int* macro_twosharps(const int* ptr0) {
  ...
  for (ptr = ptr0;;) {
    TOK_GET(&t, &ptr, &cval);
    if (t == 0) break;
    if (t == TOK_PPJOIN) continue;
    while (*ptr == TOK_PPJOIN) {
      int t1;
      CValue cv1;
      /* given 'a##b', remove nosubsts preceding 'a' */
      if (start_of_nosubsts >= 0) macro_str1.len = start_of_nosubsts;
    ...
    if (t == TOK_NOSUBST) {
      if (start_of_nosubsts < 0) start_of_nosubsts = macro_str1.len;
    } else {
      start_of_nosubsts = -1;
    }
    tok_str_add2(&macro_str1, t, &cval);
    ...
    if (macro_ptr) {
      ...
      if (ws_str) {
        while (is_space(ch) || ch == '\n' || ch == '/') {
          ...
          if (ch == '\n') file->line_num++;
          if (!(ch == '\f' || ch == '\v' || ch == '\r')) tok_str_add(ws_str, ch);
          PEEKC(ch, p);
        }
      }
      file->buf_ptr = p;
...
static int macro_subst_tok(TokenString* tok_str, Sym** nested_list, Sym* s) {
  ...
  if (tok == TOK___LINE__ || tok == TOK___COUNTER__) {
    ...
    tm = localtime(&ti);
    if (tok == TOK___DATE__) {
      snprintf(buf, sizeof(buf), "%s %2d %d", ab_month_name[tm->tm_mon], tm->tm_mday,
               tm->tm_year + 1900);
    } else {
  ...
  add_cstr1:
    ...
    if (s->type.t == MACRO_FUNC) {
      ...
      if (t != '(') {
        ...
        if (parse_flags & PARSE_FLAG_SPACES) {
          int i;
          for (i = 0; i < ws_str.len; i++) tok_str_add(tok_str, ws_str.str[i]);
        }
      ...
      for (;;) {
        ...
        } while (tok == TOK_PLCHLDR || is_space(tok) || TOK_LINEFEED == tok);
      empty_arg:
        /* handle '()' case */
        if (!args && !sa && tok == ')') break;
        if (!sa) tcc_error("macro '%s' used with too many args", get_tok_str(s->v, 0));
        tok_str_new(&str);
        parlevel = spc = 0;
        /* NOTE: non zero sa->t indicates VA_ARGS */
        while ((parlevel > 0 || (tok != ')' && (tok != ',' || sa->type.t)))) {
          if (tok == TOK_EOF || tok == 0) break;
          if (tok == '(')
            parlevel++;
          else if (tok == ')')
            parlevel--;
          if (tok == TOK_LINEFEED) tok = ' ';
          if (!check_space(tok, &spc)) tok_str_add2(&str, tok, &tokc);
          next_argstream(nested_list, NULL);
        }
        if (parlevel) expect(")");
        str.len -= spc;
        ...
        if (tok == ')') {
          ...
          if (sa && sa->type.t && gnu_ext) goto empty_arg;
          break;
        }
        if (tok != ',') expect(",");
      }
    ...
    *nested_list = sa1->prev;
    sym_free(sa1);
    if (joined_str) tok_str_free_str(joined_str);
    if (mstr != s->d) tok_str_free_str(mstr);
  }
  return 0;
}
...
static void macro_subst(TokenString* tok_str, Sym** nested_list, const int* macro_str) {
  ...
  while (1) {
    ...
    no_subst:
      if (!check_space(t, &spc)) tok_str_add2(tok_str, t, &cval);

      if (nosubst) {
        if (nosubst > 1 && (spc || (++nosubst == 3 && t == '('))) continue;
        nosubst = 0;
      }
      if (t == TOK_NOSUBST) nosubst = 1;
    }
    /* GCC supports 'defined' as result of a macro substitution */
    if (t == TOK_DEFINED && pp_expr) nosubst = 2;
  }
}
...
static void next_nomacro(void) {
  ...
  redo:
    ...
    if (TOK_HAS_VALUE(t)) {
      ...
      macro_ptr++;
      if (t < TOK_IDENT) {
        if (!(parse_flags & PARSE_FLAG_SPACES) && (isidnum_table[t - CH_EOF] & IS_SPC)) goto redo;
      }
      tok = t;
    }
  } else {
...
redo:
  ...
  if (macro_ptr) {
    if (!TOK_HAS_VALUE(t)) {
      if (t == TOK_NOSUBST || t == TOK_PLCHLDR) {
        ...
        if (!(parse_flags & PARSE_FLAG_ACCEPT_STRAYS)) tcc_error("stray '\\' in program");
      }
      return;
    }
  ...
  if (t == TOK_PPNUM) {
    ...
    if (parse_flags & PARSE_FLAG_TOK_STR) parse_string((char*)tokc.str.data, tokc.str.size - 1);
  }
}
...
static void putdefs(CString* cs, const char* p) {
  while (*p) putdef(cs, p), p = strchr(p, 0) + 1;
}
...
static void tcc_predefs(TCCState* s1, CString* cs, int is_asm) {
  ...
  if (s1->float_abi == ARM_HARD_FLOAT) putdef(cs, "__ARM_PCS_VFP");
#endif
  if (is_asm) putdef(cs, "__ASSEMBLER__");
  if (s1->output_type == TCC_OUTPUT_PREPROCESS) putdef(cs, "__TCC_PP__");
  if (s1->output_type == TCC_OUTPUT_MEMORY) putdef(cs, "__TCC_RUN__");
  if (s1->char_is_unsigned) putdef(cs, "__CHAR_UNSIGNED__");
  if (s1->optimize > 0) putdef(cs, "__OPTIMIZE__");
  if (s1->option_pthread) putdef(cs, "_REENTRANT");
  if (s1->leading_underscore) putdef(cs, "__leading_underscore");
#ifdef CONFIG_TCC_BCHECK
  if (s1->do_bounds_check) putdef(cs, "__BOUNDS_CHECKING_ON");
#endif
#ifdef CONFIG_TCC_BACKTRACE
  if (s1->do_backtrace) putdef(cs, "__TCC_BACKTRACE_ENABLED__");
#endif
...
ST_FUNC void preprocess_start(TCCState* s1, int filetype) {
  ...
  if (!(filetype & AFF_TYPE_ASM)) {
    ...
    if (s1->cmdline_defs.size) cstr_cat(&cstr, s1->cmdline_defs.data, s1->cmdline_defs.size);
    if (s1->cmdline_incl.size) cstr_cat(&cstr, s1->cmdline_incl.data, s1->cmdline_incl.size);
    // printf("%s\n", (char*)cstr.data);
...
ST_FUNC void tccpp_delete(TCCState* s) {
  ...
  if (n > total_idents) total_idents = n;
  for (i = 0; i < n; i++) tal_free(toksym_alloc, table_ident[i]);
...
static void pp_debug_builtins(TCCState* s1) {
  int v;
  for (v = TOK_IDENT; v < tok_ident; ++v) define_print(s1, v);
}
...
static int pp_check_he0xE(int t, const char* p) {
  if (t == TOK_PPNUM && toup(strchr(p, 0)[-1]) == 'E') return 'E';
  return t;
}
...
ST_FUNC int tcc_preprocess(TCCState* s1) {
  ...
  if (s1->Pflag == LINE_MACRO_OUTPUT_FORMAT_P10) parse_flags |= PARSE_FLAG_TOK_NUM, s1->Pflag = 1;
  ...
  if (file->prev) pp_line(s1, file->prev, level++);
