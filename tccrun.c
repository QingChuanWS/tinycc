...
LIBTCCAPI int tcc_relocate(TCCState* s1, void* ptr) {
  ...
  if (TCC_RELOCATE_AUTO != ptr) return tcc_relocate_ex(s1, ptr, 0);
...
#ifdef HAVE_SELINUX
  {
    ...
    if (ptr == MAP_FAILED || prx == MAP_FAILED) tcc_error("tccrun: could not map memory");
...
static void run_cdtors(TCCState* s1, const char* start, const char* end, int argc, char** argv,
                       char** envp) {
  void** a = (void**)get_sym_addr(s1, start, 0, 0);
  void** b = (void**)get_sym_addr(s1, end, 0, 0);
  while (a != b) ((void (*)(int, char**, char**)) * a++)(argc, argv, envp);
}
...
LIBTCCAPI int tcc_run(TCCState* s1, int argc, char** argv) {
  ...
  if ((s1->dflag & 16) && (addr_t)-1 == get_sym_addr(s1, s1->runtime_main, 0, 1)) return 0;
  tcc_add_symbol(s1, "exit", run_exit);
  tcc_add_symbol(s1, "atexit", rt_atexit);
  tcc_add_symbol(s1, "on_exit", rt_on_exit);
  if (tcc_relocate(s1, TCC_RELOCATE_AUTO) < 0) return -1;
  ...
  if (s1->do_debug) {
    ...
    if (s1->dwarf) {
      ...
      if (dwarf_line_str_section) rc->dwarf_line_str = dwarf_line_str_section->data;
    } else {
    ...
    if (s1->dwarf) rc->prog_base = (addr_t)-1;
#else
#endif
#endif
    ...
    rc->do_jmp = 1;
    if ((p = tcc_get_symbol(s1, "__rt_error"))) *(void**)p = _rt_error;
#ifdef CONFIG_TCC_BCHECK
    if (s1->do_bounds_check) {
      rc->bounds_start = (void*)bounds_section->sh_addr;
      if ((p = tcc_get_symbol(s1, "__bound_init"))) ((void (*)(void*, int))p)(rc->bounds_start, 1);
    }
#endif
  ...
  call_exit(ret);
  if ((s1->dflag & 16) && ret) fprintf(s1->ppfp, "[returns %d]\n", ret), fflush(s1->ppfp);
  if ((s1->dflag & 16) == 0 && (&g_exit_context)->exit_called) exit(ret);
  return ret;
}
...
redo:
  for (k = 0; k < 3; ++k) { /* 0:rx, 1:ro, 2:rw sections */
    ...
    for (i = 1; i < s1->nb_sections; i++) {
      static const char shf[] = {SHF_ALLOC | SHF_EXECINSTR, SHF_ALLOC, SHF_ALLOC | SHF_WRITE};
      s = s1->sections[i];
      if (shf[k] != (s->sh_flags & (SHF_ALLOC | SHF_WRITE | SHF_EXECINSTR))) continue;
      length = s->data_offset;
      if (copy) {
        if (addr == 0) addr = s->sh_addr;
        n = (s->sh_addr - addr) + length;
        ptr = (void*)s->sh_addr;
        if (k == 0) ptr = (void*)(s->sh_addr - ptr_diff);
        if (NULL == s->data || s->sh_type == SHT_NOBITS)
          memset(ptr, 0, length);
        else
          memcpy(ptr, s->data, length);
#ifdef _WIN64
        if (s == s1->uw_pdata) *(void**)mem = win64_add_function_table(s1);
#endif
        if (s->data) {
          tcc_free(s->data);
          s->data = NULL;
      ...
      if (++n == 1 && align < (PAGE_ALIGN - 1)) align = (PAGE_ALIGN - 1);
...
#if DEBUG_RUNMEN
      if (mem)
        printf("%d: %-16s %p  len %04x  align %04x\n", k, s->name, (void*)s->sh_addr, length,
               align + 1);
#endif
    }
    if (copy) {                         /* set permissions */
      if (k == 0 && ptr_diff) continue; /* not with HAVE_SELINUX */
      f = k;
#if !CONFIG_RUNMEM_RO
      if (f != 0) continue;
...
#endif
      if (n) set_pages_executable(s1, f, (void*)addr, n);
    }
  }
  ...
  if (s1->nb_errors) return -1;
  if (0 == mem) return offset + max_align;
...
static void set_pages_executable(TCCState* s1, int mode, void* ptr, unsigned long length) {
#ifdef _WIN32
  static const unsigned char protect[] = {PAGE_EXECUTE_READ, PAGE_READONLY, PAGE_READWRITE,
                                          PAGE_EXECUTE_READWRITE};
  DWORD old;
  VirtualProtect(ptr, length, protect[mode], &old);
#else
  static const unsigned char protect[] = {PROT_READ | PROT_EXEC, PROT_READ, PROT_READ | PROT_WRITE,
...
static void* win64_add_function_table(TCCState* s1) {
  ...
  if (s1->uw_pdata) {
    ...
    RtlAddFunctionTable((RUNTIME_FUNCTION*)p, s1->uw_pdata->data_offset / sizeof(RUNTIME_FUNCTION),
                        s1->pe_imagebase);
    s1->uw_pdata = NULL;
  }
  return p;
...
static char* rt_elfsym(rt_context* rc, addr_t wanted_pc, addr_t* func_addr) {
  ...
  for (esym = rc->esym_start + 1; esym < rc->esym_end; ++esym) {
    ...
    if ((type == STT_FUNC || type == STT_GNU_IFUNC) && wanted_pc >= esym->st_value &&
...
static addr_t rt_printline(rt_context* rc, addr_t wanted_pc, const char* msg, const char* skip) {
  char func_name[128];
...
next:
  ...
  for (sym = rc->stab_sym + 1; sym < rc->stab_sym_end; ++sym) {
    ...
    switch (sym->n_type) {
      ...
      rel_pc:
        pc += func_addr;
      check_pc:
        if (pc >= wanted_pc && wanted_pc >= last_pc) goto found;
        break;
    }
    ...
    switch (sym->n_type) {
      ...
      case N_FUN:
        if (sym->n_strx == 0) goto reset_func;
        p = strchr(str, ':');
        if (0 == p || (len = p - str + 1, len > sizeof func_name)) len = sizeof func_name;
      ...
      case N_SLINE:
        ...
        /* include files */
      case N_BINCL:
        if (incl_index < INCLUDE_STACK_SIZE) incl_files[incl_index++] = str;
        break;
      case N_EINCL:
        if (incl_index > 1) incl_index--;
        break;
      ...
      case N_SO:
        ...
        if (sym->n_strx) {
          /* do not add path */
          len = strlen(str);
          if (len > 0 && str[len - 1] != '/') incl_files[incl_index++] = str;
        }
      reset_func:
        func_name[0] = '\0';
        func_addr = 0;
      ...
      case N_SOL:
        if (incl_index) incl_files[incl_index - 1] = str;
        break;
    }
  }
  ...
  if ((rc = rc->next)) goto next;
found:
  i = last_incl_index;
  if (i > 0) {
    str = incl_files[--i];
    if (skip[0] && strstr(str, skip)) return (addr_t)-1;
...
#define dwarf_read_2(ln, end) ((ln) + 2 < (end) ? (ln) += 2, read16le((ln)-2) : 0)
#define dwarf_read_4(ln, end) ((ln) + 4 < (end) ? (ln) += 4, read32le((ln)-4) : 0)
#define dwarf_read_8(ln, end) ((ln) + 8 < (end) ? (ln) += 8, read64le((ln)-8) : 0)
...
static unsigned long long dwarf_read_uleb128(unsigned char** ln, unsigned char* end) {
  unsigned char* cp = *ln;
...
static long long dwarf_read_sleb128(unsigned char** ln, unsigned char* end) {
  unsigned char* cp = *ln;
  long long retval = 0;
  int i;
  ...
  for (i = 0; i < MAX_128; i++) {
    ...
    retval |= (byte & 0x7f) << (i * 7);
    if ((byte & 0x80) == 0) {
      if ((byte & 0x40) && (i + 1) * 7 < 64) retval |= -1LL << ((i + 1) * 7);
      break;
    }
  }
  *ln = cp;
...
static addr_t rt_printline_dwarf(rt_context* rc, addr_t wanted_pc, const char* msg,
                                 const char* skip) {
  unsigned char* ln;
  unsigned char* cp;
  unsigned char* end;
...
next:
  ...
  while (ln < rc->dwarf_line_end) {
    ...
    if (end < ln || end > rc->dwarf_line_end) break;
    ...
    if (version >= 5) {
      ...
      for (i = 0; i < dir_size; i++) {
        for (j = 0; j < col; j++) {
          if (entry_format[j].type == DW_LNCT_path) {
            if (entry_format[j].form != DW_FORM_line_strp) goto next_line;
#if 0
...
#else
            length == 4 ? dwarf_read_4(ln, end) : dwarf_read_8(ln, end);
#endif
          } else
      ...
      for (i = 0; i < filename_size; i++)
        for (j = 0; j < col; j++) {
          if (entry_format[j].type == DW_LNCT_path) {
            if (entry_format[j].form != DW_FORM_line_strp) goto next_line;
            value = length == 4 ? dwarf_read_4(ln, end) : dwarf_read_8(ln, end);
            if (i < FILE_TABLE_SIZE) filename_table[i].name = (char*)rc->dwarf_line_str + value;
            ...
            if (i < FILE_TABLE_SIZE) filename_table[i].dir_entry = value;
          } else
            dwarf_ignore_type(ln, end);
        }
    } else {
      ...
      while ((dwarf_read_1(ln, end))) {
        if (++filename_size < FILE_TABLE_SIZE) {
          ...
          filename_table[filename_size - 1].dir_entry = dwarf_read_uleb128(&ln, end);
        } else {
          while (dwarf_read_1(ln, end)) {
          }
          dwarf_read_uleb128(&ln, end);
    ...
    if (filename_size >= 1) filename = filename_table[0].name;
    while (ln < end) {
      last_pc = pc;
      ...
      check_pc:
        ...
        switch (i) {
...
#endif
#if defined TCC_TARGET_MACHO
                if (rc->prog_base != (addr_t)-1) pc += rc->prog_base;
#endif
                opindex = 0;
                break;
          ...
          case DW_LNS_set_file:
            ...
            i -= i > 0 && version < 5;
            if (i < FILE_TABLE_SIZE && i < filename_size) filename = filename_table[i].name;
            break;
          ...
          case DW_LNS_fixed_advance_pc:
            ...
            goto check_pc;
          default:
            for (j = 0; j < opcode_length[i - 1]; j++) dwarf_read_uleb128(&ln, end);
            break;
        }
      }
    }
  ...
  if (function) goto found;
  if ((rc = rc->next)) goto next;
found:
  if (filename) {
    if (skip[0] && strstr(filename, skip)) return (addr_t)-1;
...
static int _rt_error(void* fp, void* ip, const char* fmt, va_list ap) {
  ...
  for (i = level = 0; level < n; i++) {
    ...
    if (level == 0) {
      ...
      break;
    rt_printf("\n");
    if (ret == -1 || (pc == (addr_t)rc->top_func && pc)) break;
    ++level;
  }
...
static void rt_exit(int code) {
  rt_context* rc = &g_rtctxt;
  if (rc->do_jmp) longjmp(rc->jmp_buf, code ? code : 256);
  exit(code);
}
...
static long __stdcall cpu_exception_handler(EXCEPTION_POINTERS* ex_info) {
  ...
  if (rc->do_jmp) rt_exit(255);
  return EXCEPTION_EXECUTE_HANDLER;
}

/* Generate a stack backtrace when a CPU exception occurs. */
static void set_exception_handler(void) { SetUnhandledExceptionFilter(cpu_exception_handler); }
...
static int rt_get_caller_pc(addr_t* paddr, rt_context* rc, int level) {
  ...
  if (level == 0) {
    ...
    if (fp > 0x1000) ip = ((addr_t*)fp)[1];
  }
  if (ip <= 0x1000) return -1;
  *paddr = ip;
  return 0;
}
...
static int rt_get_caller_pc(addr_t* paddr, rt_context* rc, int level) {
  if (level == 0) {
    *paddr = rc->ip;
    ...
    addr_t* fp = (addr_t*)rc->fp;
    while (--level && fp >= (addr_t*)0x1000) fp = (addr_t*)fp[-2];
    if (fp < (addr_t*)0x1000) return -1;
    *paddr = fp[-1];
  }
  return 0;
}
...
static int rt_get_caller_pc(addr_t* paddr, rt_context* rc, int level) { return -1; }
...
ST_FUNC void* dlsym(void* handle, const char* symbol) {
  ...
  p = tcc_syms;
  while (p->str != NULL) {
    if (!strcmp(p->str, symbol)) return p->ptr;
    p++;
  }
  return NULL;
}
