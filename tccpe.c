...
static const char* pe_export_name(TCCState* s1, ElfW(Sym) * sym) {
  ...
  if (s1->leading_underscore && name[0] == '_' && !(sym->st_other & ST_PE_STDCALL)) return name + 1;
  return name;
}
...
#define CHARACTERISTICS_DLL 0x230F
  ...
  pe->sizeofheaders =
      pe_file_align(pe, sizeof(struct pe_header) + pe->sec_count * sizeof(IMAGE_SECTION_HEADER));
  ...
  for (i = 0; i < pe->sec_count; ++i) {
    ...
    switch (si->cls) {
      case sec_text:
        if (!pe_header.opthdr.BaseOfCode) pe_header.opthdr.BaseOfCode = addr;
        break;

      case sec_data:
#ifndef TCC_TARGET_X86_64
        if (!pe_header.opthdr.BaseOfData) pe_header.opthdr.BaseOfData = addr;
#endif
        break;
    ...
    if (pe->imp_size) {
      ...
      pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_IAT, pe->iat_offs, pe->iat_size);
    }
    if (pe->exp_size) {
      pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_EXPORT, pe->exp_offs, pe->exp_size);
    }
  ...
  if (pe->s1->pe_stack_size) pe_header.opthdr.SizeOfStackReserve = pe->s1->pe_stack_size;
  if (PE_DLL == pe->type) pe_header.filehdr.Characteristics = CHARACTERISTICS_DLL;
...
static void pe_build_imports(struct pe_info* pe) {
  ...
  for (sym_cnt = i = 0; i < ndlls; ++i) sym_cnt += pe->imp_info[i]->sym_count;
  ...
  for (i = 0; i < pe->imp_count; ++i) {
    ...
    for (k = 0, n = p->sym_count; k <= n; ++k) {
      if (k < n) {
        ...
        if (pe->type == PE_RUN) {
          if (dllref) {
            if (!dllref->handle) dllref->handle = LoadLibrary(dllref->name);
            v = (ADDR3264)GetProcAddress(dllref->handle, ordinal ? (char*)0 + ordinal : name);
          }
          if (!v) tcc_error_noabort("could not resolve symbol '%s'", name);
        } else
#endif
            if (ordinal) {
      ...
      *(ADDR3264*)(pe->thunk->data + thk_ptr) = *(ADDR3264*)(pe->thunk->data + ent_ptr) = v;
...
static void pe_build_exports(struct pe_info* pe) {
  ...
  if (NULL == op) {
    ...
    if (pe->s1->verbose) printf("<- %s (%d symbol%s)\n", buf, sym_count, &"s"[sym_count < 2]);
  }
#endif
...
static void pe_build_reloc(struct pe_info* pe) {
  ...
  for (;;) {
    if (rel < rel_end) {
      ...
      ++rel;
      if (type != REL_TYPE_DIRECT) continue;
...
static int pe_section_class(Section* s) {
  ...
  if (0 == memcmp(name, ".stab", 5)) {
    if (0 == s->s1->do_debug) return sec_last;
  ...
  if (flags & SHF_ALLOC) {
    if (type == SHT_PROGBITS || type == SHT_INIT_ARRAY || type == SHT_FINI_ARRAY) {
      ...
      if (0 == strcmp(name, ".rsrc")) return sec_rsrc;
      if (0 == strcmp(name, ".iedat")) return sec_idata;
      if (0 == strcmp(name, ".pdata")) return sec_pdata;
      return sec_rdata;
    ...
    if (0 == strcmp(name, ".reloc")) return sec_reloc;
  }
  return sec_last;
}
...
static int pe_assign_addresses(struct pe_info* pe) {
  ...
  if (PE_DLL == pe->type) pe->reloc = new_section(pe->s1, ".reloc", SHT_PROGBITS, 0);
  ...
  for (i = 1; (c = sec_cls[i]) < sec_last; ++i) {
    ...
    if (PE_MERGE_DATA && c == sec_bss) c = sec_data;
    ...
    if (s == pe->reloc) pe_build_reloc(pe);

    if (0 == s->data_offset) continue;

    if (si) goto add_section;
    ...
    if (s->sh_flags & SHF_WRITE) si->pe_flags |= IMAGE_SCN_MEM_WRITE;
    if (0 == (s->sh_flags & SHF_ALLOC)) si->pe_flags |= IMAGE_SCN_MEM_DISCARDABLE;
...
static int pe_isafunc(TCCState* s1, int sym_index) {
  ...
  for (rel = (ElfW_Rel*)sr->data; rel < rel_end; rel++) {
    if (rel->r_info == info) return 1;
#ifdef R_XXX_FUNCCALL2
    if (rel->r_info == info2) return 1;
#endif
  }
  return 0;
}
...
static int pe_check_symbols(struct pe_info* pe) {
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
      ...
      if (type == STT_FUNC) {
        ...
        if (offset) {
          ...
          is->iat_index = put_elf_sym(symtab_section, 0, sizeof(DWORD),
                                      ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0, SHN_UNDEF, buffer);
          ...
          put_elf_reloc(symtab_section, text_section, offset + 8, R_XXX_THUNKFIX,
...
#endif
          put_elf_reloc(symtab_section, text_section, offset + 2, R_XXX_THUNKFIX, is->iat_index);
#endif
        }
    ...
    not_found:
      if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK) /* STB_WEAK undefined symbols are accepted */
        continue;
...
static void pe_print_section(FILE* f, Section* s) {
  ...
  if (s->sh_type == SHT_SYMTAB || s->sh_type == SHT_RELX) {
    const char* fields1[] = {"name", "value", "size", "bind", "type", "other", "shndx", NULL};
  ...
  for (i = 0; i < l;) {
    ...
    if (s->sh_type == SHT_SYMTAB) {
      ...
      fprintf(f, "  %04X  %04X  %04X   %02X    %02X    %02X   %04X  \"%s\"", (unsigned)sym->st_name,
              (unsigned)sym->st_value, (unsigned)sym->st_size,
              (unsigned)ELFW(ST_BIND)(sym->st_info), (unsigned)ELFW(ST_TYPE)(sym->st_info),
              (unsigned)sym->st_other, (unsigned)sym->st_shndx, name);
      ...
      fprintf(f, "  %04X   %02X   %04X  \"%s\"", (unsigned)rel->r_offset,
              (unsigned)ELFW(R_TYPE)(rel->r_info), (unsigned)ELFW(R_SYM)(rel->r_info), name);
    } else {
      fprintf(f, "   ");
      for (n = 0; n < m; ++n) {
        if (n + i < l) {
...
ST_FUNC SValue* pe_getimport(SValue* sv, SValue* v2) {
  int r2;
  if ((sv->r & (VT_VALMASK | VT_SYM)) != (VT_CONST | VT_SYM) || (sv->r2 != VT_CONST)) return sv;
  if (!sv->sym->a.dllimport) return sv;
...
ST_FUNC int pe_putimport(TCCState* s1, int dllindex, const char* name, addr_t value) {
  return set_elf_sym(s1->dynsymtab_section, value, dllindex, /* st_size */
                     ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, value ? SHN_ABS : SHN_UNDEF, name);
}
...
static int get_dllexports(int fd, char** pp) {
  ...
  p = NULL;
  ret = 1;
  if (!read_mem(fd, 0, &dh, sizeof dh)) goto the_end;
  if (!read_mem(fd, dh.e_lfanew, &sig, sizeof sig)) goto the_end;
  if (sig != 0x00004550) goto the_end;
  pef_hdroffset = dh.e_lfanew + sizeof sig;
  if (!read_mem(fd, pef_hdroffset, &ih, sizeof ih)) goto the_end;
  opt_hdroffset = pef_hdroffset + sizeof ih;
  if (ih.Machine == 0x014C) {
    IMAGE_OPTIONAL_HEADER32 oh;
    sec_hdroffset = opt_hdroffset + sizeof oh;
    if (!read_mem(fd, opt_hdroffset, &oh, sizeof oh)) goto the_end;
    if (IMAGE_DIRECTORY_ENTRY_EXPORT >= oh.NumberOfRvaAndSizes) goto the_end_0;
  ...
  for (i = 0; i < ih.NumberOfSections; ++i) {
    if (!read_mem(fd, sec_hdroffset + i * sizeof ish, &ish, sizeof ish)) goto the_end;
    // printf("vaddr: %08x\n", ish.VirtualAddress);
    if (addr >= ish.VirtualAddress && addr < ish.VirtualAddress + ish.SizeOfRawData) goto found;
  }
  goto the_end_0;

found:
  ref = ish.VirtualAddress - ish.PointerToRawData;
  if (!read_mem(fd, addr - ref, &ied, sizeof ied)) goto the_end;

  namep = ied.AddressOfNames - ref;
  for (i = 0; i < ied.NumberOfNames; ++i) {
    if (!read_mem(fd, namep, &ptr, sizeof ptr)) goto the_end;
    namep += sizeof ptr;
    for (l = 0;;) {
      if (n + 1 >= n0) p = tcc_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(TCCState* s1, int fd) {
  ...
  if (!read_mem(fd, 0, &hdr, sizeof hdr)) goto quit;

  if (hdr.filehdr.Machine != IMAGE_FILE_MACHINE || hdr.filehdr.NumberOfSections != 1 ||
  ...
  if (!read_mem(fd, offs, ptr, hdr.sectionhdr.SizeOfRawData)) goto quit;
  ...
  for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i) {
    ...
    if (!read_mem(fd, offs, &rel, sizeof rel)) goto quit;
    // printf("rsrc_reloc: %x %x %x\n", rel.offset, rel.size, rel.type);
    if (rel.type != RSRC_RELTYPE) goto quit;
    put_elf_reloc(symtab_section, rsrc_section, rel.offset, R_XXX_RELATIVE, sym_index);
    offs += sizeof rel;
  }
  ret = 0;
quit:
...
static int pe_load_def(TCCState* s1, int fd) {
  ...
  for (line = buf;; ++line) {
    ...
    switch (state) {
      case 0:
        ...
        break;
      case 1:
        if (0 != stricmp(p, "EXPORTS")) goto quit;
        ++state;
        break;
      case 2:
  ...
  skip:
    while ((unsigned char)next > ' ') get_token(&line, &next);
    if (next != '\n') break;
  }
  ret = 0;
quit:
...
static int pe_load_dll(TCCState* s1, int fd, const char* filename) {
  ...
  if (ret) {
    ...
    for (q = p; *q; q += 1 + strlen(q)) pe_putimport(s1, index, q, 0);
    tcc_free(p);
  }
  return 0;
}
...
static void pe_add_runtime(TCCState* s1, struct pe_info* pe) {
  ...
  if (TCC_OUTPUT_DLL == s1->output_type) {
    ...
    if (TCC_OUTPUT_MEMORY == s1->output_type) start_symbol = run_symbol;
  }

  pe->start_symbol = start_symbol + 1;
  if (!s1->leading_underscore || strchr(start_symbol, '@')) ++start_symbol;

#ifdef CONFIG_TCC_BACKTRACE
  if (s1->do_backtrace) {
#ifdef CONFIG_TCC_BCHECK
    if (s1->do_bounds_check && s1->output_type != TCC_OUTPUT_DLL) tcc_add_support(s1, "bcheck.o");
#endif
    if (s1->output_type == TCC_OUTPUT_EXE) tcc_add_support(s1, "bt-exe.o");
    if (s1->output_type == TCC_OUTPUT_DLL) tcc_add_support(s1, "bt-dll.o");
  ...
  if (0 == s1->nostdlib) {
    static const char* const libs[] = {"msvcrt", "kernel32", "", "user32", "gdi32", NULL};
  ...
  if (TCC_OUTPUT_DLL == s1->output_type) s1->output_type = TCC_OUTPUT_EXE;
  if (TCC_OUTPUT_MEMORY == s1->output_type) pe_type = PE_RUN;
  pe->type = pe_type;
}
...
static void pe_set_options(TCCState* s1, struct pe_info* pe) {
  ...
  if (s1->pe_subsystem != 0) pe->subsystem = s1->pe_subsystem;
  ...
  if (s1->section_align != 0) pe->section_align = s1->section_align;
  if (s1->pe_file_align != 0) pe->file_align = s1->pe_file_align;

  if ((pe->subsystem >= 10) && (pe->subsystem <= 12)) pe->imagebase = 0;

  if (s1->has_text_addr) pe->imagebase = s1->text_addr;
}
...
#if PE_PRINT_SECTIONS
  if (s1->g_debug & 8) pe_print_sections(s1, "tcc.log");
#endif
  return ret;
}
