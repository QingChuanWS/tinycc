...
BOOL WINAPI DllMain(HINSTANCE hDll, DWORD dwReason, LPVOID lpReserved) {
  if (DLL_PROCESS_ATTACH == dwReason) tcc_module = hDll;
  return TRUE;
}
#else
...
ST_FUNC void wait_sem(TCCSem* p) {
  if (!p->init) InitializeCriticalSection(&p->cr), p->init = 1;
  EnterCriticalSection(&p->cr);
}
ST_FUNC void post_sem(TCCSem* p) { LeaveCriticalSection(&p->cr); }
...
ST_FUNC void wait_sem(TCCSem* p) {
  if (!p->init) p->sem = dispatch_semaphore_create(1), p->init = 1;
  dispatch_semaphore_wait(p->sem, DISPATCH_TIME_FOREVER);
}
ST_FUNC void post_sem(TCCSem* p) { dispatch_semaphore_signal(p->sem); }
#else
ST_FUNC void wait_sem(TCCSem* p) {
  if (!p->init) sem_init(&p->sem, 0, 1), p->init = 1;
  while (sem_wait(&p->sem) < 0 && errno == EINTR)
    ;
}
ST_FUNC void post_sem(TCCSem* p) { sem_post(&p->sem); }
#endif
#endif

PUB_FUNC void tcc_enter_state(TCCState* s1) {
  if (s1->error_set_jmp_enabled) return;
...
PUB_FUNC void* tcc_malloc(unsigned long size) {
  void* ptr;
  ptr = malloc(size);
  if (!ptr && size) _tcc_error("memory full (malloc)");
  return ptr;
}
...
PUB_FUNC void* tcc_malloc_debug(unsigned long size, const char* file, int line) {
  ...
  if (!header) _tcc_error("memory full (malloc)");
...
PUB_FUNC void tcc_free_debug(void* ptr) {
  ...
  if (header == mem_debug_chain) mem_debug_chain = header->next;
  free(header);
}
...
static void tcc_split_path(TCCState* s, void* p_ary, int* p_nb_ary, const char* in) {
  ...
  do {
    ...
    for (p = in; c = *p, c != '\0' && c != PATHSEP[0]; ++p) {
      if (c == '{' && p[1] && p[2] == '}') {
        c = p[1], p += 2;
        if (c == 'B') cstr_cat(&str, s->tcc_lib_path, -1);
        if (c == 'R') cstr_cat(&str, CONFIG_SYSROOT, -1);
        if (c == 'f' && file) {
...
static void error1(int mode, const char* fmt, va_list ap) {
  ...
  if (s1 == NULL) /* can happen only if called from tcc_malloc(): 'out of memory' */
    goto no_file;

  tcc_exit_state(s1);

  if (mode == ERROR_WARN) {
    if (s1->warn_error) mode = ERROR_ERROR;
    if (s1->warn_num) {
      /* handle tcc_warning_c(warn_option)(fmt, ...) */
      int wopt = *(&s1->warn_none + s1->warn_num);
      s1->warn_num = 0;
      if (0 == (wopt & WARN_ON)) return;
      if (wopt & WARN_ERR) mode = ERROR_ERROR;
      if (wopt & WARN_NOE) mode = ERROR_WARN;
    }
  ...
  if (f) {
    ...
    cstr_printf(&cs, "%s:%d: ", f->filename, f->line_num - !!(tok_flags & TOK_FLAG_BOL));
  } else if (s1->current_filename) {
    cstr_printf(&cs, "%s: ", s1->current_filename);
  }

no_file:
  if (0 == cs.size) cstr_printf(&cs, "tcc: ");
  ...
  if (s1) {
    ...
    if (mode != ERROR_ERROR) return;
    if (s1->error_set_jmp_enabled) longjmp(s1->error_jmp_buf, 1);
  }
  exit(1);
}
...
ST_FUNC void tcc_close(void) {
  ...
  if (bf->true_filename != bf->filename) tcc_free(bf->true_filename);
  file = bf->prev;
  tcc_free(bf);
}
...
static int _tcc_open(TCCState* s1, const char* filename) {
  ...
  if ((s1->verbose == 2 && fd >= 0) || s1->verbose == 3)
    printf("%s %*s%s\n", fd < 0 ? "nf" : "->", (int)(s1->include_stack_ptr - s1->include_stack), "",
           filename);
  return fd;
}
...
LIBTCCAPI void tcc_define_symbol(TCCState* s1, const char* sym, const char* value) {
  const char* eq;
  if (NULL == (eq = strchr(sym, '='))) eq = strchr(sym, 0);
  if (NULL == value) value = *eq ? eq + 1 : "1";
...
#if TARGETOS_OpenBSD
    if (output_type != TCC_OUTPUT_DLL) tcc_add_crt(s, "crt0.o");
    if (output_type == TCC_OUTPUT_DLL)
      tcc_add_crt(s, "crtbeginS.o");
    else
      tcc_add_crt(s, "crtbegin.o");
#elif TARGETOS_FreeBSD
    if (output_type != TCC_OUTPUT_DLL) tcc_add_crt(s, "crt1.o");
...
ST_FUNC DLLReference* tcc_add_dllref(TCCState* s1, const char* dllname, int level) {
  ...
  if (level == -1) return ref;
  if (ref) {
    if (level < ref->level) ref->level = level;
    ref->found = 1;
    return ref;
  }
...
static int tcc_glob_so(TCCState* s1, const char* pattern, char* buf, int size) {
  ...
  star = strchr(pattern, '*');
  if (!star || glob(pattern, 0, NULL, &g)) return -1;
  for (v = -1, i = 0; i < g.gl_pathc; ++i) {
    p = g.gl_pathv[i];
    if (2 != sscanf(p + (star - pattern), "%d.%d.%d", &v1, &v2, &v3)) continue;
    if ((v1 = v1 * 1000 + v2) > v) v = v1, pstrcpy(buf, size, p);
  }
  globfree(&g);
  return v;
}
...
#if defined TARGETOS_OpenBSD && !defined _WIN32
  char buf[1024];
  if (tcc_glob_so(s1, filename, buf, sizeof buf) >= 0) filename = buf;
#endif

  /* ignore binary files with -E */
  if (s1->output_type == TCC_OUTPUT_PREPROCESS && (flags & AFF_TYPE_BIN)) return 0;

  /* open the file */
  fd = _tcc_open(s1, filename);
  if (fd < 0) {
    if (flags & AFF_PRINT_ERROR) tcc_error_noabort("file '%s' not found", filename);
    return ret;
  }
    ...
    switch (obj_type) {
      ...
      case_dyn_or_tbd:
        if (s1->output_type == TCC_OUTPUT_MEMORY) {
          ...
          if (obj_type != AFF_BINTYPE_DYN) soname = macho_tbd_soname(filename);
          dl = dlopen(soname, RTLD_GLOBAL | RTLD_LAZY);
          if (dl) tcc_add_dllref(s1, soname, 0)->handle = dl, ret = 0;
          if (filename != soname) tcc_free((void*)soname);
#endif
      ...
      default: {
        ...
        if (!strcmp(ext, ".tbd")) goto case_dyn_or_tbd;
        if (!strcmp(ext, ".dylib")) {
      ...
      case AFF_BINTYPE_DYN:
        if (s1->output_type == TCC_OUTPUT_MEMORY) {
          ...
          if (dl) tcc_add_dllref(s1, filename, 0)->handle = dl, ret = 0;
#endif
        } else
      ...
      check_success:
        if (ret < 0) tcc_error_noabort("%s: unrecognized file type", filename);
        break;
...
static int tcc_add_library_internal(TCCState* s, const char* fmt, const char* filename, int flags,
  ...
  for (i = 0; i < nb_paths; i++) {
    ...
    if (tcc_add_file_internal(s, buf, flags | AFF_TYPE_BIN) == 0) return 0;
  }
  return -1;
}
...
ST_FUNC void tcc_add_support(TCCState* s1, const char* filename) {
  char buf[100];
  if (CONFIG_TCC_CROSSPREFIX[0]) filename = strcat(strcpy(buf, CONFIG_TCC_CROSSPREFIX), filename);
  if (tcc_add_dll(s1, filename, 0) < 0) tcc_error_noabort("%s not found", filename);
}

#if !defined TCC_TARGET_PE && !defined TCC_TARGET_MACHO
ST_FUNC int tcc_add_crt(TCCState* s1, const char* filename) {
  if (-1 == tcc_add_library_internal(s1, "%s/%s", filename, 0, s1->crt_paths, s1->nb_crt_paths))
...
LIBTCCAPI int tcc_add_library(TCCState* s, const char* libraryname) {
  ...
  static const char* const libs[] = {"%s/%s.def",    "%s/lib%s.def", "%s/%s.dll",
...
PUB_FUNC int tcc_add_library_err(TCCState* s1, const char* libname) {
  ...
  if (ret < 0) tcc_error_noabort("library '%s' not found", libname);
  return ret;
}
...
ST_FUNC void tcc_add_pragma_libs(TCCState* s1) {
  int i;
  for (i = 0; i < s1->nb_pragma_libs; i++) tcc_add_library_err(s1, s1->pragma_libs[i]);
}
...
static int tcc_set_linker(TCCState* s, const char* option) {
  ...
  while (*option) {
    ...
    if (ignoring) tcc_warning_c(warn_unsupported)("unsupported linker option '%s'", option);
...
static int set_flag(TCCState* s, const FlagDef* flags, const char* name) {
  ...
  for (ret = -1, p = flags; p->name; ++p) {
    if (ret) {
      if (strcmp(r, p->name)) continue;
    } else {
      if (0 == (p->flags & WD_ALL)) continue;
    }
...
/* read list file */
static void args_parser_listfile(TCCState* s, const char* filename, int optind, int* pargc,
                                 char*** pargv) {
  TCCState* s1 = s;
  int fd, i;
  char* p;
  ...
  fd = open(filename, O_RDONLY | O_BINARY);
  if (fd < 0) tcc_error("listfile '%s' not found", filename);
...
static uint32_t parse_version(TCCState* s1, const char* version) {
  ...
  if (*last == '.') {
    ...
    if (*last == '.') c = strtoul(&last[1], &last, 10);
  }
...
PUB_FUNC int tcc_parse_args(TCCState* s, int* pargc, char*** pargv, int optind) {
  ...
  reparse:
    ...
    for (popt = tcc_options;; ++popt) {
      ...
      const char* r1 = r + 1;
      if (p1 == NULL) tcc_error("invalid option -- '%s'", r);
    ...
    switch (popt->index) {
      ...
      case TCC_OPTION_static:
        ...
        break;
      case TCC_OPTION_std:
        if (strcmp(optarg, "=c11") == 0) s->cversion = 201112;
        break;
      ...
      case TCC_OPTION_v:
        ...
        break;
      case TCC_OPTION_f:
        if (set_flag(s, options_f, optarg) < 0) goto unsupported_option;
        break;
#ifdef TCC_TARGET_ARM
      ...
      case TCC_OPTION_m:
        if (set_flag(s, options_m, optarg) < 0) {
          if (x = atoi(optarg), x != 32 && x != 64) goto unsupported_option;
          if (PTR_SIZE != x / 8) return x;
          ++noaction;
        }
        break;
      case TCC_OPTION_W:
        s->warn_none = 0;
        if (optarg[0] && set_flag(s, options_W, optarg) < 0) goto unsupported_option;
        break;
      case TCC_OPTION_w:
        s->warn_none = 1;
        break;
      case TCC_OPTION_rdynamic:
        ...
        break;
      case TCC_OPTION_Wl:
        if (linker_arg.size) --linker_arg.size, cstr_ccat(&linker_arg, ',');
        cstr_cat(&linker_arg, optarg, 0);
        if (tcc_set_linker(s, linker_arg.data)) cstr_free(&linker_arg);
        break;
      case TCC_OPTION_Wp:
        r = optarg;
        goto reparse;
      ...
      case TCC_OPTION_MM:
        s->just_deps = 1;
        if (!s->deps_outfile) s->deps_outfile = tcc_strdup("-");
        // fall through
      case TCC_OPTION_MMD:
        s->gen_deps = 1;
        break;
      ...
      extra_action:
        arg_start = optind - 1;
        if (arg_start != noaction) tcc_error("cannot parse %s here", r);
        tool = x;
        break;
      default:
      unsupported_option:
  ...
  *pargv = argv + arg_start;
  ...
  if (s->verbose == 2) return OPT_PRINT_DIRS;
  if (s->verbose) return OPT_V;
  return OPT_HELP;
}
...
PUB_FUNC void tcc_print_stats(TCCState* s1, unsigned total_time) {
  ...
  fprintf(stderr,
          ...
          (unsigned)total_lines * 1000 / total_time, (double)total_bytes / 1000 / total_time);
