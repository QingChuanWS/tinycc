...
ST_DATA int global_expr; /* true if compound literals must be allocated globally (used during
...
static void decl_initializer_alloc(CType* type, AttributeDef* ad, int r, int has_init, int v,
                                   int scope);
...
static int gind() {
  int t = ind;
  CODE_ON();
  if (debug_modes) tcc_tcov_block_begin(tcc_state);
  return t;
}
...
static int R_RET(int t) {
  if (!is_float(t)) return REG_IRET;
#ifdef TCC_TARGET_X86_64
  if ((t & VT_BTYPE) == VT_LDOUBLE) return TREG_ST0;
#elif defined TCC_TARGET_RISCV64
  if ((t & VT_BTYPE) == VT_LDOUBLE) return REG_IRET;
#endif
  return REG_FRET;
}
...
static int R2_RET(int t) {
  t &= VT_BTYPE;
#if PTR_SIZE == 4
  if (t == VT_LLONG) return REG_IRE2;
#elif defined TCC_TARGET_X86_64
  if (t == VT_QLONG) return REG_IRE2;
  if (t == VT_QFLOAT) return REG_FRE2;
#elif defined TCC_TARGET_RISCV64
  if (t == VT_LDOUBLE) return REG_IRE2;
#endif
  return VT_CONST;
}
...
static void PUT_R_RET(SValue* sv, int t) { sv->r = R_RET(t), sv->r2 = R2_RET(t); }

/* returns function return register class for type t */
static int RC_RET(int t) { return reg_classes[R_RET(t)] & ~(RC_FLOAT | RC_INT); }

/* returns generic register class for type t */
static int RC_TYPE(int t) {
  if (!is_float(t)) return RC_INT;
#ifdef TCC_TARGET_X86_64
  if ((t & VT_BTYPE) == VT_LDOUBLE) return RC_ST0;
  if ((t & VT_BTYPE) == VT_QFLOAT) return RC_FRET;
#elif defined TCC_TARGET_RISCV64
  if ((t & VT_BTYPE) == VT_LDOUBLE) return RC_INT;
#endif
  return RC_FLOAT;
}
...
static int RC2_TYPE(int t, int rc) {
  if (!USING_TWO_WORDS(t)) return 0;
#ifdef RC_IRE2
  if (rc == RC_IRET) return RC_IRE2;
#endif
#ifdef RC_FRE2
  if (rc == RC_FRET) return RC_FRE2;
#endif
  if (rc & RC_FLOAT) return RC_FLOAT;
  return RC_INT;
}
...
ST_FUNC void tccgen_init(TCCState* s1) {
  ...
  char_type.t = VT_BYTE;
  if (s1->char_is_unsigned) char_type.t |= VT_UNSIGNED;
...
#ifdef TCC_TARGET_PE
  if (sym->a.dllimport) esym->st_other |= ST_PE_IMPORT;
  if (sym->a.dllexport) esym->st_other |= ST_PE_EXPORT;
#endif
...
ST_FUNC void put_extern_sym2(Sym* sym, int sh_num, addr_t value, unsigned long size,
  ...
  if (!sym->c) {
    ...
    if ((t & VT_BTYPE) == VT_FUNC) {
      ...
      sym_type = STT_NOTYPE;
      if ((t & (VT_BTYPE | VT_ASM_FUNC)) == VT_ASM_FUNC) sym_type = STT_FUNC;
    } else {
      sym_type = STT_OBJECT;
    }
    ...
    if (debug_modes) tcc_debug_extern_sym(tcc_state, sym, sh_num, sym_bind, sym_type);
...
ST_FUNC void put_extern_sym(Sym* sym, Section* s, addr_t value, unsigned long size) {
  if (nocode_wanted && (NODATA_WANTED || (s && s == cur_text_section))) return;
...
ST_FUNC void greloca(Section* s, Sym* sym, unsigned long offset, int type, addr_t addend) {
  ...
  if (nocode_wanted && s == cur_text_section) return;

  if (sym) {
    if (0 == sym->c) put_extern_sym(sym, NULL, 0, 0);
    c = sym->c;
  }
...
ST_FUNC Sym* global_identifier_push(int v, int t, int c) {
  ...
  if (v < SYM_FIRST_ANOM) {
    ...
    while (*ps != NULL && (*ps)->sym_scope) ps = &(*ps)->prev_tok;
    s->prev_tok = *ps;
    *ps = s;
  }
  return s;
...
static void vcheck_cmp(void) {
  ...
  if (vtop->r == VT_CMP && !nocode_wanted) gv(RC_INT);
}

static void vsetc(CType* type, int r, CValue* vc) {
  if (vtop >= vstack + (VSTACK_SIZE - 1)) tcc_error("memory full (vstack)");
  vcheck_cmp();
  vtop++;
  vtop->type = *type;
  vtop->r = r;
...
ST_FUNC void vpush_helper_func(int v) { vpushsym(&func_old_type, external_helper_sym(v)); }

/* Merge symbol attributes.  */
static void merge_symattr(struct SymAttr* sa, struct SymAttr* sa1) {
  if (sa1->aligned && !sa->aligned) sa->aligned = sa1->aligned;
  sa->packed |= sa1->packed;
  sa->weak |= sa1->weak;
  if (sa1->visibility != STV_DEFAULT) {
    int vis = sa->visibility;
    if (vis == STV_DEFAULT || vis > sa1->visibility) vis = sa1->visibility;
    sa->visibility = vis;
  }
...
static void merge_funcattr(struct FuncAttr* fa, struct FuncAttr* fa1) {
  if (fa1->func_call && !fa->func_call) fa->func_call = fa1->func_call;
  if (fa1->func_type && !fa->func_type) fa->func_type = fa1->func_type;
  if (fa1->func_args && !fa->func_args) fa->func_args = fa1->func_args;
  if (fa1->func_noreturn) fa->func_noreturn = 1;
  if (fa1->func_ctor) fa->func_ctor = 1;
  if (fa1->func_dtor) fa->func_dtor = 1;
}
...
static void merge_attr(AttributeDef* ad, AttributeDef* ad1) {
  ...
  if (ad1->section) ad->section = ad1->section;
  if (ad1->alias_target) ad->alias_target = ad1->alias_target;
  if (ad1->asm_label) ad->asm_label = ad1->asm_label;
  if (ad1->attr_mode) ad->attr_mode = ad1->attr_mode;
}

/* Merge some type attributes.  */
static void patch_type(Sym* sym, CType* type) {
  if (!(type->t & VT_EXTERN) || IS_ENUM_VAL(sym->type.t)) {
    if (!(sym->type.t & VT_EXTERN)) tcc_error("redefinition of '%s'", get_tok_str(sym->v, NULL));
...
static void patch_storage(Sym* sym, AttributeDef* ad, CType* type) {
  ...
  if (ad->asm_label) sym->asm_label = ad->asm_label;
  update_storage(sym);
}
...
static Sym* external_sym(int v, CType* type, int r, AttributeDef* ad) {
  ...
  s = sym_find(v);
  while (s && s->sym_scope) s = s->prev_tok;
  ...
  if (!s) {
    ...
    if (local_stack) sym_copy_ref(s, &global_stack);
  } else {
    patch_storage(s, ad, type);
  }
  /* push variables on local_stack if any */
  if (local_stack && (s->type.t & VT_BTYPE) != VT_FUNC) s = sym_copy(s, &local_stack);
  return s;
}
...
ST_FUNC void save_regs(int n) {
  SValue *p, *p1;
  for (p = vstack, p1 = vtop - n; p <= p1; p++) save_reg(p->r);
}
...
ST_FUNC void save_reg_upstack(int r, int n) {
  ...
  if ((r &= VT_VALMASK) >= VT_CONST) return;
  ...
  for (p = vstack, p1 = vtop - n; p <= p1; p++) {
    if ((p->r & VT_VALMASK) == r || p->r2 == r) {
      ...
      if (!l) {
        ...
        if (bt == VT_VOID) continue;
        if ((p->r & VT_LVAL) || bt == VT_FUNC) bt = VT_PTR;
        sv.type.t = bt;
...
ST_FUNC int get_reg(int rc) {
  ...
  for (r = 0; r < NB_REGS; r++) {
    if (reg_classes[r] & rc) {
      if (nocode_wanted) return r;
      for (p = vstack; p <= vtop; p++) {
        if ((p->r & VT_VALMASK) == r || p->r2 == r) goto notfound;
      }
      return r;
    }
  notfound:;
...
/* find a free temporary local variable (return the offset on stack) match the size and align. If
 * none, add new temporary stack variable*/
static int get_temp_local_var(int size, int align) {
  int i;
...
ST_FUNC void gaddrof(void) {
  ...
  if ((vtop->r & VT_VALMASK) == VT_LLOCAL) vtop->r = (vtop->r & ~VT_VALMASK) | VT_LOCAL | VT_LVAL;
}
...
#if defined TCC_TARGET_I386 || defined TCC_TARGET_X86_64
    if (v == TOK_alloca) func_bound_add_epilog = 1;
#endif
#if TARGETOS_NetBSD
...
static void add_local_bounds(Sym* s, Sym* e) {
  for (; s != e; s = s->prev) {
    if (!s->v || (s->r & VT_VALMASK) != VT_LOCAL) continue;
...
#endif
  if (debug_modes) tcc_add_debug_info(tcc_state, !local_scope, local_stack, b);
...
static void load_packed_bf(CType* type, int bit_pos, int bit_size) {
  ...
  do {
    ...
    if (bit_pos) vpushi(bit_pos), gen_op(TOK_SHR), bit_pos = 0;  // X B Y
...
ST_FUNC int gv(int rc) {
  ...
  if (vtop->type.t & VT_BITFIELD) {
    ...
    if ((vtop->type.t & VT_BTYPE) == VT_BOOL) type.t |= VT_UNSIGNED;
    ...
    /* XXX mega hack */
    if (bt == VT_LDOUBLE && rc == RC_FLOAT) rc = RC_INT;
#endif
    rc2 = RC2_TYPE(bt, rc);
    ...
    if (!r_ok || !r2_ok) {
      ...
      if (rc2) {
        ...
        if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
          ...
          /* move registers */
          if (!r_ok) load(r, vtop);
          if (r2_ok && vtop->r2 < VT_CONST) goto done;
          vdup();
          vtop[-1].r = r; /* save register value */
          vtop->r = vtop[-1].r2;
        }
...
static void gen_opic(int op) {
  ...
  if (c1 && c2) {
    ...
    if (c1 && (op == '+' || op == '&' || op == '^' || op == '|' || op == '*' || op == TOK_EQ ||
               op == TOK_NE)) {
      vswap();
    ...
    if (!const_wanted && c1 &&
      ...
      vswap();
      vtop--;
    } else if (c2 && (((op == '*' || op == '/' || op == TOK_UDIV || op == TOK_PDIV) && l2 == 1) ||
      ...
      if ((int)l2 != l2) goto general_case;
      vtop--;
      vtop->c.i = l2;
    } else {
    general_case:
  ...
  if (vtop->r == VT_CONST) vtop->r |= nonconst;
}
...
static void gen_opif(int op) {
  ...
  if (c1 && c2) {
    ...
    if (!(ieee_finite(f1) || !ieee_finite(f2)) && !const_wanted) goto general_case;
    switch (op) {
      case '+':
        f1 += f2;
        break;
      ...
      case '/':
        if (f2 == 0.0) {
          ...
          if (!const_wanted) goto general_case;
...
static void type_to_str(char* buf, int buf_size, CType* type, const char* varstr) {
  ...
  if (t & VT_EXTERN) pstrcat(buf, buf_size, "extern ");
  if (t & VT_STATIC) pstrcat(buf, buf_size, "static ");
  if (t & VT_TYPEDEF) pstrcat(buf, buf_size, "typedef ");
  if (t & VT_INLINE) pstrcat(buf, buf_size, "inline ");
  if (bt != VT_PTR) {
  ...
  switch (bt) {
    ...
    case VT_LLONG:
      tstr = "long long";
    maybe_long:
      if (t & VT_LONG) tstr = "long";
      if (!IS_ENUM(t)) goto add_tstr;
      tstr = "enum ";
      goto tstruct;
    case VT_FLOAT:
      tstr = "float";
    ...
    case VT_FUNC:
      ...
      while (sa != NULL) {
        ...
        sa = sa->next;
        if (sa) pstrcat(buf1, sizeof(buf1), ", ");
      }
      if (s->f.func_type == FUNC_ELLIPSIS) pstrcat(buf1, sizeof(buf1), ", ...");
      pstrcat(buf1, sizeof(buf1), ")");
    ...
    case VT_PTR:
      ...
      if (t & VT_CONSTANT) pstrcat(buf1, buf_size, "const ");
      if (t & VT_VOLATILE) pstrcat(buf1, buf_size, "volatile ");
...
static inline int is_null_pointer(SValue* p) {
  if ((p->r & (VT_VALMASK | VT_LVAL | VT_SYM | VT_NONCONST)) != VT_CONST) return 0;
...
static int is_compatible_func(CType* type1, CType* type2) {
  ...
  for (;;) {
    ...
    if (s1->f.func_type == FUNC_OLD || s2->f.func_type == FUNC_OLD) return 1;
    s1 = s1->next;
    s2 = s2->next;
    if (!s1) return !s2;
    if (!s2) return 0;
...
static int compare_types(CType* type1, CType* type2, int unqualified) {
  ...
  if ((t1 & VT_ARRAY) &&
      !(type1->ref->c < 0 || type2->ref->c < 0 || type1->ref->c == type2->ref->c))
    return 0;
  ...
  if (bt1 == VT_VOID || bt2 == VT_VOID) {
    ...
    /* If one is a null ptr constant the result type is the other.  */
      ...
      /* accept comparison or cond-expr between pointer and integer
         with a warning */
      if ((op == '?' || TOK_ISCOND(op)) && (is_integer_btype(bt1) || is_integer_btype(bt2)))
      ...
      if (pbt1 != VT_VOID && pbt2 != VT_VOID && !compare_types(pt1, pt2, 1 /*unqualif*/)) {
        if (op != '?' && !TOK_ISCOND(op))
          ...
          type_incompatibility_warning(
              type1, type2,
              op == '?' ? "pointer type mismatch in conditional expression ('%s' and '%s')"
      ...
      if (op == '?') {
        ...
        if (pt1->t & VT_ARRAY && pt2->t & VT_ARRAY && pointed_type(&type)->ref->c < 0 &&
          ...
          if (!copied) type.ref = sym_push(SYM_FIELD, &type.ref->type, 0, type.ref->c);
    ...
    if (TOK_ISCOND(op)) type.t = VT_SIZE_T;
  } else if (bt1 == VT_STRUCT || bt2 == VT_STRUCT) {
    if (op != '?' || !compare_types(type1, type2, 1)) ret = 0;
    type = *type1;
    ...
    if (bt1 == VT_LLONG) type.t &= t1;
    if (bt2 == VT_LLONG) type.t &= t2;
...
redo:
  ...
  if (bt1 == VT_FUNC || bt2 == VT_FUNC) {
    ...
    int align;
    if (TOK_ISCOND(op)) goto std_op;
    /* if both pointers, then it must be the '-' op */
    if (bt1 == VT_PTR && bt2 == VT_PTR) {
      if (op != '-') tcc_error("cannot use pointers here");
      ...
      if (op != '-' && op != '+') tcc_error("cannot use pointers here");
    ...
    if (is_float(combtype.t) && op != '+' && op != '-' && op != '*' && op != '/' && !TOK_ISCOND(op))
      tcc_error("invalid operands for binary operation");
    else if (op == TOK_SHR || op == TOK_SAR || op == TOK_SHL) {
      t = bt1 == VT_LLONG ? VT_LLONG : VT_INT;
      if ((t1 & (VT_BTYPE | VT_UNSIGNED | VT_BITFIELD)) == (t | VT_UNSIGNED)) t |= VT_UNSIGNED;
      t |= (VT_LONG & t1);
      combtype.t = t;
    }
  std_op:
    ...
    /* special case for shifts and long long: we keep the shift as
       an integer */
    if (op == TOK_SHR || op == TOK_SAR || op == TOK_SHL) t2 = VT_INT;
    gen_cast_s(t2);
    if (is_float(t))
      gen_opif(op);
    else
  ...
  if (vtop->r & VT_LVAL) gv(is_float(vtop->type.t & VT_BTYPE) ? RC_FLOAT : RC_INT);
}
...
static void gen_cvt_itof1(int t) {
  if ((vtop->type.t & (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG | VT_UNSIGNED)) {
    if (t == VT_FLOAT) vpush_helper_func(TOK___floatundisf);
#if LDOUBLE_SIZE != 8
...
static void gen_cvt_ftoi1(int t) {
  ...
  if (t == (VT_LLONG | VT_UNSIGNED)) {
    ...
    if (st == VT_FLOAT) vpush_helper_func(TOK___fixunssfdi);
#if LDOUBLE_SIZE != 8
...
static void gen_cast(CType* type) {
  ...
  /* special delayed cast for char/short */
  if (vtop->r & VT_MUSTCAST) force_charshort_cast();

  /* bitfields first get cast to ints */
  if (vtop->type.t & VT_BITFIELD) gv(RC_INT);

  dbt = type->t & (VT_BTYPE | VT_UNSIGNED);
  sbt = vtop->type.t & (VT_BTYPE | VT_UNSIGNED);
  if (sbt == VT_FUNC) sbt = VT_PTR;
...
again:
  if (sbt != dbt) {
    ...
    if (nocode_wanted & DATA_ONLY_WANTED) goto done;
    ...
    if (sf || df) {
      if (sf && df) {
        ...
        /* convert fp to int */
        sbt = dbt;
        if (dbt_bt != VT_LLONG && dbt_bt != VT_INT) sbt = VT_INT;
        gen_cvt_ftoi1(sbt);
    ...
    if (ds == 0 || ss == 0) goto error;

    if (IS_ENUM(type->t) && type->ref->c < 0) tcc_error("cast to incomplete type");

    /* same size and no sign conversion needed */
    if (ds == ss && ds >= 4) goto done;
...
static void vpush_type_size(CType* type, int* a) {
  if (type->t & VT_VLA) {
    ...
    if (size < 0) tcc_error("unknown type size");
#if PTR_SIZE == 8
    vpushll(size);
#else
    vpushi(size);
...
static void verify_assign_cast(CType* dt) {
  ...
  dbt = dt->t & VT_BTYPE;
  sbt = st->t & VT_BTYPE;
  if (dt->t & VT_CONSTANT) tcc_warning("assignment of read-only location");
  switch (dbt) {
    case VT_VOID:
      if (sbt != dbt) tcc_error("assignment to void expression");
      break;
    case VT_PTR:
      /* special cases for pointers */
      /* '0' can also be a pointer */
      if (is_null_pointer(vtop)) break;
      ...
      if (sbt == VT_PTR)
        ...
        goto error;
      if (is_compatible_types(type1, type2)) break;
      for (qualwarn = lvl = 0;; ++lvl) {
        ...
        if (dbt != VT_PTR || sbt != VT_PTR) break;
        type1 = pointed_type(type1);
        type2 = pointed_type(type2);
      }
      if (!is_compatible_unqualified_types(type1, type2)) {
        if ((dbt == VT_VOID || sbt == VT_VOID) && lvl == 0) {
          /* void * can match anything */
                   ...
                   IS_ENUM(type1->t) + IS_ENUM(type2->t) + !!((type1->t ^ type2->t) & VT_UNSIGNED) <
                       2) {
...
ST_FUNC void vstore(void) {
  ...
  if (sbt == VT_STRUCT) {
    ...
    vpushv(vtop - 1);
#ifdef CONFIG_TCC_BCHECK
    if (vtop->r & VT_MUSTBOUND) gbound(); /* check would be wrong after gaddrof() */
#endif
    vtop->type.t = VT_PTR;
    gaddrof();
    /* source */
    ...
    if ((dbt == VT_BYTE || dbt == VT_SHORT) && is_integer_btype(sbt)) {
      if ((vtop->r & VT_MUSTCAST) && btype_size(dbt) > btype_size(sbt)) force_charshort_cast();
      delayed_cast = 1;
    } else {
      gen_cast(&vtop[-1].type);
    }
...
redo:
  if (tok != TOK_ATTRIBUTE1 && tok != TOK_ATTRIBUTE2) return;
  next();
  skip('(');
  skip('(');
  while (tok != ')') {
    ...
    switch (t) {
      ...
      case TOK_VISIBILITY2:
        skip('(');
        parse_mult_str(&astr, "visibility(\"default|hidden|internal|protected\")");
      ...
      case TOK_ALIGNED2:
        if (tok == '(') {
          next();
          n = expr_const();
          if (n <= 0 || (n & (n - 1)) != 0) tcc_error("alignment must be a positive power of two");
          skip(')');
        } else {
          n = MAX_ALIGN;
        }
      ...
      case TOK_REGPARM2:
        ...
        if (n > 3)
          ...
          n = 0;
        if (n > 0) ad->f.func_call = FUNC_FASTCALL1 + n - 1;
        skip(')');
        break;
      case TOK_FASTCALL1:
      case TOK_FASTCALL2:
...
static Sym* find_field(CType* type, int v, int* cumofs) {
  ...
  if (!(v & SYM_FIELD)) { /* top-level call */
    ...
    if (s->c < 0)
      ...
      tcc_error("field not found: %s", get_tok_str(v, &tokc));
  }
  return NULL;
}
...
static void check_fields(CType* type, int check) {
  ...
  while ((s = s->next) != NULL) {
    ...
    if (v < SYM_FIRST_ANOM) {
      ...
      if (check && (ts->tok & SYM_FIELD)) tcc_error("duplicate member '%s'", get_tok_str(v, NULL));
      ts->tok ^= SYM_FIELD;
...
static void struct_layout(CType* type, AttributeDef* ad) {
  ...
  for (f = type->ref->next; f; f = f->next) {
    ...
    if (type->ref->type.t == VT_UNION) {
      if (pcc && bit_size >= 0) size = (bit_size + 7) >> 3;
      offset = 0;
      if (size > c) c = size;

    } else if (bit_size < 0) {
      if (pcc) c += (bit_pos + 7) >> 3;
      c = (c + align - 1) & -align;
      offset = c;
      if (size > 0) c += size;
      bit_pos = 0;
      ...
      if (pcc) {
        ...
        new_field:
          ...
          if (ofs > size / align) goto new_field;
        }

        /* in pcc mode, long long bitfields have type int if they fit */
        if (size == 8 && bit_size <= 32) f->type.t = (f->type.t & ~VT_BTYPE) | VT_INT, size = 4;

        while (bit_pos >= align * 8) c += align, bit_pos -= align * 8;
        offset = c;
        ...
        if ((bit_pos + bit_size > size * 8) || (bit_size > 0) == (bt != prevbt)) {
          ...
          if (bit_size || prev_bit_size) c += size;
        }
        /* In MS layout the records alignment is normally
           ...
           fields it is again).  */
        if (bit_size == 0 && prevbt != bt) align = 1;
        prevbt = bt;
...
#ifdef BF_DEBUG
    printf("set field %s offset %-2d size %-2d align %-2d", get_tok_str(f->v & ~SYM_FIELD, NULL),
           offset, size, align);
    if (f->type.t & VT_BITFIELD) {
  ...
  for (f = type->ref->next; f; f = f->next) {
    ...
    if (0 == (f->type.t & VT_BITFIELD)) continue;
    f->type.ref = f;
    f->auxtype = -1;
    bit_size = BIT_SIZE(f->type.t);
...
static void struct_decl(CType* type, int u) {
  ...
  if (tok != '{') {
    ...
    if (s && (s->sym_scope == local_scope || tok != '{')) {
      if (u == s->type.t) goto do_decl;
      if (u == VT_ENUM && IS_ENUM(s->type.t)) goto do_decl;
  ...
  if (tok == '{') {
    ...
    if (u == VT_ENUM) {
      ...
      for (;;) {
        ...
        ss = sym_find(v);
        if (ss && !local_stack) tcc_error("redefinition of enumerator '%s'", get_tok_str(v, NULL));
        next();
        if (tok == '=') {
          next();
          ll = expr_const64();
      ...
      t.t = VT_INT;
      if (nl >= 0) {
        if (pl != (unsigned)pl) t.t = (LONG_SIZE == 8 ? VT_LLONG | VT_LONG : VT_LLONG);
        t.t |= VT_UNSIGNED;
      ...
      while (tok != '}') {
        ...
        while (1) {
          ...
          type1 = btype;
          if (tok != ':') {
            if (tok != ';') type_decl(&type1, &ad1, &v, TYPE_DIRECT);
            if (v == 0) {
              if ((type1.t & VT_BTYPE) != VT_STRUCT)
                ...
                int v = btype.ref->v;
                if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
                  if (tcc_state->ms_extensions == 0) expect("identifier");
                }
              }
            }
            ...
            if ((type1.t & VT_BTYPE) == VT_FUNC || (type1.t & VT_BTYPE) == VT_VOID ||
                (type1.t & VT_STORAGE))
          ...
          if (tok == ':') {
            ...
            if (bit_size < 0) tcc_error("negative width in bit-field '%s'", get_tok_str(v, NULL));
            if (v && bit_size == 0)
          ...
          if (bit_size >= 0) {
            ...
            if (bit_size > bsize) {
              ...
              type1.t =
                  (type1.t & ~VT_STRUCT_MASK) | VT_BITFIELD | (bit_size << (VT_STRUCT_SHIFT + 6));
            }
          }
      ...
      if (debug_modes) tcc_debug_fix_anon(tcc_state, type);
    }
  }
}
    ...
    switch (tok) {
      ...
      basic_type1:
        if (u == VT_SHORT || u == VT_LONG) {
          ...
          if (bt != -1 || (st != -1 && u != VT_INT)) goto tmbt;
          bt = u;
        }
        if (u != VT_INT) t = (t & ~(VT_BTYPE | VT_LONG)) | u;
        typespec_found = 1;
        break;
      case TOK_VOID:
        u = VT_VOID;
      ...
      case TOK__Atomic:
        ...
        if (tok == '(') {
          ...
          if (type1.ref) sym_to_attr(ad, type1.ref);
          goto basic_type2;
        }
        break;
      case TOK_CONST1:
      ...
      case TOK_TYPEDEF:
        ...
        goto storage;
      storage:
        if (t & (VT_EXTERN | VT_STATIC | VT_TYPEDEF) & ~g) tcc_error("multiple storage classes");
        t |= g;
        next();
        break;
      case TOK_INLINE1:
      ...
      default:
        if (typespec_found) goto the_end;
        s = sym_find(tok);
        if (!s || !(s->type.t & VT_TYPEDEF)) goto the_end;
        ...
        type->ref = s->type.ref;
        if (t) parse_btype_qualify(type, t);
        t = type->t;
...
the_end:
  if (tcc_state->char_is_unsigned) {
    if ((t & (VT_DEFSIGN | VT_BTYPE)) == VT_BYTE) t |= VT_UNSIGNED;
  }
  /* VT_LONG is used just as a modifier for VT_INT / VT_LLONG */
  bt = t & (VT_BTYPE | VT_LONG);
  if (bt == VT_LONG) t |= LONG_SIZE == 8 ? VT_LLONG : VT_INT;
#ifdef TCC_USING_DOUBLE_FOR_LDOUBLE
  if (bt == VT_LDOUBLE) t = (t & ~(VT_BTYPE | VT_LONG)) | (VT_DOUBLE | VT_LONG);
#endif
  type->t = t;
  return type_found;
}
...
static int post_type(CType* type, AttributeDef* ad, int storage, int td) {
  ...
  if (tok == '(') {
    ...
    next();
    if (TYPE_DIRECT == (td & (TYPE_DIRECT | TYPE_ABSTRACT))) return 0;
    if (tok == ')')
      l = 0;
    ...
    if (l) {
      for (;;) {
        ...
        if (l != FUNC_OLD) {
          ...
          if ((pt.t & VT_BTYPE) == VT_VOID) tcc_error("parameter declared as void");
          if (n == 0) n = SYM_FIELD;
        } else {
          n = tok;
        ...
        *plast = s;
        ...
        if (l == FUNC_NEW && !parse_btype(&pt, &ad1, 0)) tcc_error("invalid type");
      }
    } else
    ...
    if (first) {
      ...
      for (s = first; s; s = s->next) s->v |= SYM_FIELD;
    }
    --local_scope;
    ...
    check:
      if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
        n = vtop->c.i;
        if (n < 0) tcc_error("invalid array size");
      } else {
    ...
    if ((type->t & VT_BTYPE) == VT_FUNC) tcc_error("declaration of an array of functions");
    ...
    if (t1 & VT_VLA) {
      if (n < 0) {
        if (td & TYPE_NEST) tcc_error("need explicit inner array size in VLAs");
      } else {
...
static CType* type_decl(CType* type, AttributeDef* ad, int* v, int td) {
  ...
  redo:
    ...
    mk_pointer(type);
    type->t |= qualifiers;
    if (ret == type) /* innermost pointed to type is the one for the first derivation */
...
static void parse_atomic(int atok) {
  ...
  for (arg = 0;;) {
    ...
    switch (template[arg]) {
      ...
      case 'A':
        atom_ptr = &vtop->type;
        if ((atom_ptr->t & VT_BTYPE) != VT_PTR) expect("pointer");
...
ST_FUNC void unary(void) {
  ...
  if (debug_modes) tcc_debug_line(tcc_state), tcc_tcov_check_line(tcc_state, 1);
...
tok_next:
  switch (tok) {
    ...
    case TOK___FUNCTION__:
      if (!gnu_ext) goto tok_identifier;
      /* fall thru */
    case TOK___FUNC__: {
      Section* sec;
      int len;
      ...
      type.t = char_type.t;
      if (tcc_state->warn_write_strings & WARN_ON) type.t |= VT_CONSTANT;
      mk_pointer(&type);
      type.t |= VT_ARRAY;
      type.ref->c = len;
      sec = rodata_section;
      vpush_ref(&type, sec, sec->data_offset, len);
      if (!NODATA_WANTED) memcpy(section_ptr_add(sec, len), funcname, len);
      next();
    } break;
    case TOK_LSTR:
#ifdef TCC_TARGET_PE
    ...
    case TOK_STR:
      ...
      t = char_type.t;
    str_init:
      if (tcc_state->warn_write_strings & WARN_ON) t |= VT_CONSTANT;
      type.t = t;
      mk_pointer(&type);
      type.t |= VT_ARRAY;
      memset(&ad, 0, sizeof(AttributeDef));
    ...
    case '(':
      ...
      if (parse_btype(&type, &ad, 0)) {
        ...
        int saved_nocode_wanted = nocode_wanted;
        if (const_wanted && !(nocode_wanted & unevalmask)) expect("constant");
        if (0 == local_scope) tcc_error("statement expression outside of function");
        /* save all registers */
        save_regs(0);
        ...
        if (saved_nocode_wanted) nocode_wanted = saved_nocode_wanted;
        skip(')');
      } else {
        gexpr();
        skip(')');
    ...
    case '&':
      ...
      if ((vtop->type.t & VT_BTYPE) != VT_FUNC && !(vtop->type.t & (VT_ARRAY | VT_VLA)))
        test_lvalue();
      if (vtop->sym) vtop->sym->a.addrtaken = 1;
      mk_pointer(&vtop->type);
      gaddrof();
      break;
    case '!':
    ...
    case '+':
      next();
      unary();
      if ((vtop->type.t & VT_BTYPE) == VT_PTR) tcc_error("pointer not accepted for unary plus");
    ...
    case TOK_ALIGNOF3:
      ...
      if (t == TOK_SIZEOF) {
        ...
        s = NULL;
        if (vtop[1].r & VT_SYM) s = vtop[1].sym; /* hack: accessing previous vtop */
        if (s && s->a.aligned) align = 1 << (s->a.aligned - 1);
        vpushs(align);
      }
      break;
    ...
    case TOK_builtin_return_address: {
      ...
      level = expr_const64();
      if (level < 0) {
        tcc_error("%s only takes positive integers", tok1 == TOK_builtin_return_address
    ...
    case TOK_builtin_va_start:
      ...
      if (r != VT_LOCAL) tcc_error("__builtin_va_start expects a local variable");
      gen_va_start();
      vstore();
      break;
#endif
    ...
    case TOK_GENERIC: {
      ...
      if ((controlling_type.t & VT_BTYPE) == VT_FUNC) mk_pointer(&controlling_type);
      const_wanted = saved_const_wanted;
      for (;;) {
        learn = 0;
        skip(',');
        if (tok == TOK_DEFAULT) {
          if (has_default) tcc_error("too many 'default'");
          has_default = 1;
          if (!has_match) learn = 1;
          next();
        } else {
          AttributeDef ad_tmp;
          int itmp;
    ...
    tok_identifier:
      ...
      if (!s || IS_ASM_SYM(s)) {
        ...
        if (tok != '(') tcc_error("'%s' undeclared", name);
      ...
      if ((r & VT_VALMASK) < VT_CONST) r = (r & ~VT_VALMASK) | VT_LOCAL;
  ...
  while (1) {
    if (tok == TOK_INC || tok == TOK_DEC) {
      ...
      if (tok == TOK_CDOUBLE) expect("field name");
      next();
      if (tok == TOK_CINT || tok == TOK_CUINT) expect("field name");
      ...
      if ((s->type.t & VT_BTYPE) == VT_STRUCT) {
        ...
        if (ret_nregs <= 0) {
          ...
          if (size < 16)
            while (size & (size - 1)) size = (size | (size - 1)) + 1;
#endif
          loc = (loc - size) & -align;
          ret.type = s->type;
          ...
          vseti(VT_LOCAL, loc);
#ifdef CONFIG_TCC_BCHECK
          if (tcc_state->do_bounds_check) --loc;
#endif
          ret.c = vtop->c;
          if (ret_nregs < 0)
            vtop--;
      ...
      if (sa) tcc_error("too few arguments to function");
      skip(')');
      gfunc_call(nb_args);
      ...
      if (ret_nregs < 0) {
        ...
        if (((s->type.t & VT_BTYPE) == VT_STRUCT) && ret_nregs) {
          ...
          if (regsize > align) align = regsize;
          loc = (loc - size) & -align;
          addr = loc;
          offset = 0;
          for (;;) {
...
static void expr_land(void) {
  expr_or();
  if (tok == TOK_LAND) expr_landor(tok);
}

static void expr_lor(void) {
  expr_land();
  if (tok == TOK_LOR) expr_landor(tok);
}
...
static int precedence(int tok) {
  switch (tok) {
    ...
    case '%':
      return 10;
    default:
      if (tok >= TOK_ULE && tok <= TOK_GT) goto relat;
      return 0;
  }
}
...
static void init_prec(void) {
  int i;
  for (i = 0; i < 256; i++) prec[i] = precedence(i);
}
...
static void expr_infix(int p) {
  ...
  while ((p2 = precedence(t)) >= p) {
    if (t == TOK_LOR || t == TOK_LAND) {
      ...
      next();
      unary();
      if (precedence(tok) > p2) expr_infix(p2 + 1);
      gen_op(t);
    }
    t = tok;
  }
...
static void expr_cond(void) {
  ...
  if (tok == '?') {
    ...
    if (c == 0) nocode_wanted++;
    if (!g) gexpr();

    if ((vtop->type.t & VT_BTYPE) == VT_FUNC) mk_pointer(&vtop->type);
    ...
    if (USING_TWO_WORDS(type.t)) rc = RC_RET(type.t);
...
ST_FUNC int expr_const(void) {
  ...
  c = wc;
  if (c != wc && (unsigned)c != wc) tcc_error("constant exceeds 32 bit");
  return c;
}
...
static void check_func_return(void) {
  if ((func_vt.t & VT_BTYPE) == VT_VOID) return;
...
static void vla_leave(struct scope* o) {
  ...
  for (; c != o && c; c = c->prev)
    if (c->vla.num) v = c;
  if (v) vla_restore(v->vla.locorig);
}
...
static void new_scope(struct scope* o) {
  ...
  *o = *cur_scope;
  ...
  if (debug_modes) tcc_debug_stabn(tcc_state, N_LBRAC, ind - func_ind);
}

static void prev_scope(struct scope* o, int is_expr) {
  vla_leave(o->prev);

  if (o->cl.s != o->prev->cl.s) block_cleanup(o->prev);
  ...
  if (debug_modes) tcc_debug_stabn(tcc_state, N_RBRAC, ind - func_ind);
}
...
again:
  ...
  if (TOK_HAS_VALUE(t)) goto expr;
  next();

  if (debug_modes) tcc_tcov_check_line(tcc_state, 0), tcc_tcov_block_begin(tcc_state);
  ...
  if (t == TOK_IF) {
    ...
    if (tok != ';') {
      ...
      if (b) {
        ...
        if (vtop->type.t != VT_VOID) tcc_warning("void function returns a value");
        vtop--;
      }
    } else if (b) {
    ...
    if (b) gfunc_return(&func_vt);
    skip(';');
    /* jump unless last stmt in top-level block */
    if (tok != '}' || local_scope != 1) rsym = gjmp(rsym);
    if (debug_modes) tcc_tcov_block_end(tcc_state, -1);
    CODE_OFF();

  } else if (t == TOK_BREAK) {
    /* compute jump */
    if (!cur_scope->bsym) tcc_error("cannot break");
    ...
    *cur_scope->bsym = gjmp(*cur_scope->bsym);
    ...
    /* compute jump */
    if (!cur_scope->csym) tcc_error("cannot continue");
    leave_scope(loop_scope);
    *cur_scope->csym = gjmp(*cur_scope->csym);
    ...
    if (sw->nocode_wanted) goto skip_switch;
    if (sw->sv.type.t & VT_UNSIGNED)
      qsort(sw->p, sw->n, sizeof(void*), case_cmpu);
    else
      qsort(sw->p, sw->n, sizeof(void*), case_cmpi);
    for (b = 1; b < sw->n; b++)
      if (sw->sv.type.t & VT_UNSIGNED ? (uint64_t)sw->p[b - 1]->v2 >= (uint64_t)sw->p[b]->v1
  ...
  skip_switch:
    ...
    if (!cur_switch->nocode_wanted) cr->sym = gind();
    ...
    if (!cur_switch) expect("switch");
    if (cur_switch->def_sym) tcc_error("too many 'default'");
    ...
    if (cur_scope->vla.num) vla_restore(cur_scope->vla.locorig);
    if (tok == '*' && gnu_ext) {
      /* computed goto */
      next();
      gexpr();
    ...
    if (tok == ':' && t >= TOK_UIDENT) {
      ...
      s = label_find(t);
      if (s) {
        if (s->r == LABEL_DEFINED) tcc_error("duplicate label '%s'", get_tok_str(s->v, NULL));
        s->r = LABEL_DEFINED;
        if (s->next) {
          Sym* pcl; /* pending cleanup goto */
          for (pcl = s->next; pcl; pcl = pcl->prev) gsym(pcl->jnext);
    ...
    block_after_label : {
      ...
      if (debug_modes) tcc_tcov_reset_ind(tcc_state);
  ...
  if (debug_modes) tcc_tcov_check_line(tcc_state, 0), tcc_tcov_block_end(tcc_state, 0);
}
...
static void skip_or_save_block(TokenString** str) {
  int braces = tok == '{';
  int level = 0;
  if (str) *str = tok_str_alloc();

  while (1) {
    int t = tok;
    if (level == 0 && (t == ',' || t == ';' || t == '}' || t == ')' || t == ']')) break;
    if (t == TOK_EOF) {
      if (str || level > 0)
    ...
    if (str) tok_str_add_tok(*str);
    next();
    if (t == '{' || t == '(' || t == '[') {
      level++;
    } else if (t == '}' || t == ')' || t == ']') {
      level--;
      if (level == 0 && braces && t == '}') break;
    }
  }
  if (str) {
    tok_str_add(*str, -1);
...
static void decl_design_delrels(Section* sec, int c, int size) {
  ...
  while (rel < rel_end) {
    if (rel->r_offset >= c && rel->r_offset < c + size) {
      ...
      if (rel2 != rel) memcpy(rel2, rel, sizeof *rel);
      ++rel2;
    }
    ++rel;
  }
...
static int decl_designator(init_params* p, CType* type, unsigned long c, Sym** cur_field, int flags,
                           int al) {
  Sym *s, *f;
  ...
  if (flags & DIF_HAVE_ELEM) goto no_designator;

  if (gnu_ext && tok >= TOK_UIDENT) {
    l = tok, next();
    if (tok == ':') goto struct_field;
    unget_tok(l);
  }
  ...
  no_designator:
    if (type->t & VT_ARRAY) {
      ...
      if (index >= s->c) tcc_error("too many initializers");
      type = pointed_type(type);
      elem_size = type_size(type, &align);
      c += index * elem_size;
    } else {
      f = *cur_field;
      while (f && (f->v & SYM_FIRST_ANOM) && (f->type.t & VT_BITFIELD)) *cur_field = f = f->next;
      if (!f) tcc_error("too many initializers");
      type = &f->type;
      c += f->c;
    }
  }
...
static void init_putv(init_params* p, CType* type, unsigned long c) {
  ...
  if (type->t & VT_BITFIELD) size = (BIT_POS(type->t) + BIT_SIZE(type->t) + 7) / 8;
  init_assert(p, c + size);
    ...
    if ((vtop->r & (VT_SYM | VT_CONST)) == (VT_SYM | VT_CONST) && vtop->sym->v >= SYM_FIRST_ANOM &&
      ...
      if (ssec->reloc) {
        ...
        while (relofs >= sizeof(*rel)) {
          ...
          if (rel->r_offset >= esym->st_value + size) continue;
          if (rel->r_offset < esym->st_value) break;
          put_elf_reloca(symtab_section, sec, c + rel->r_offset - esym->st_value,
      ...
      if (type->t & VT_BITFIELD) {
        ...
        switch (bt) {
          ...
          case VT_PTR:
          case VT_INT:
            if (vtop->r & VT_SYM) greloc(sec, vtop->sym, c, R_DATA_PTR);
            write32le(ptr, val);
            break;
#endif
          default:
...
static void decl_initializer(init_params* p, CType* type, unsigned long c, int flags) {
  ...
  /* generate line number info */
  if (debug_modes && !p->sec) tcc_debug_line(tcc_state), tcc_tcov_check_line(tcc_state, 1);
  ...
  if (!(flags & DIF_HAVE_ELEM) && tok != '{' &&
    ...
    if ((flags & DIF_SIZE_ONLY) && !p->sec) ++nocode_wanted;
  ...
  if (type->t & VT_ARRAY) {
    ...
    if ((tok == TOK_LSTR &&
      ...
      while (tok == TOK_STR || tok == TOK_LSTR) {
        if (initstr.size) initstr.size -= size1;
        if (tok == TOK_STR)
          len += tokc.str.size;
        else
      ...
      if (!(flags & DIF_SIZE_ONLY)) {
        int nb = n, ch;
        if (len < nb) nb = len;
        if (len > nb) tcc_warning("initializer-string for array is too long");
        /* in order to go faster for common case (char
           string in global variable, we handle it
           specifically */
        if (p->sec && size1 == 1) {
          init_assert(p, c + nb);
          if (!NODATA_WANTED) memcpy(p->sec->data + c, initstr.data, nb);
        } else {
          for (i = 0; i < n; i++) {
            if (i >= nb) {
              /* only add trailing zero if enough storage (no
                 warning in this case since it is standard) */
              if (flags & DIF_CLEAR) break;
              if (n - i >= 4) {
      ...
      while (tok != '}' || (flags & DIF_HAVE_ELEM)) {
        ...
        if (type->t & VT_ARRAY) {
          ...
          /* special test for multi dimensional arrays (may not
             ...
             same time) */
          if (no_oblock && len >= n * size1) break;
        } else {
  ...
  one_elem:
    if ((flags & DIF_SIZE_ONLY)) {
      ...
      if (!(flags & DIF_HAVE_ELEM)) {
        ...
        if (tok != TOK_STR && tok != TOK_LSTR) expect("string constant");
        parse_init_elem(!p->sec ? EXPR_ANY : EXPR_CONST);
      }
...
static void decl_initializer_alloc(CType* type, AttributeDef* ad, int r, int has_init, int v,
  ...
  if (v && (r & VT_VALMASK) == VT_CONST) nocode_wanted |= DATA_ONLY_WANTED;
  ...
  if (size < 0) {
    ...
    if (!has_init) tcc_error("unknown type size");
    if (has_init == 2) {
      /* only get strings */
  ...
  if ((r & VT_VALMASK) == VT_LOCAL) {
    ...
    if (v && global) {
      ...
      if (sym) {
        ...
        if (!has_init && sym->c && elfsym(sym)->st_shndx != SHN_UNDEF) goto no_alloc;
      }
    }
  ...
  if (type->t & VT_VLA) {
    ...
    if (NODATA_WANTED) goto no_alloc;
    ...
    if (flexible_array) flexible_array->type.ref->c = -1;
  }
...
static void func_vla_arg_code(Sym* arg) {
  ...
  if (arg->type.ref) func_vla_arg_code(arg->type.ref);
...
static void func_vla_arg(Sym* sym) {
  ...
  for (arg = sym->type.ref->next; arg; arg = arg->next)
    if (arg->type.t & VT_VLA) func_vla_arg_code(arg);
}
...
static void gen_function(Sym* sym) {
  ...
  if (sym->type.ref->f.func_ctor) add_array(tcc_state, ".init_array", sym->c);
  if (sym->type.ref->f.func_dtor) add_array(tcc_state, ".fini_array", sym->c);
...
static void free_inline_functions(TCCState* s) {
  ...
  for (i = 0; i < s->nb_inline_fns; ++i) {
    ...
    if (fn->sym) tok_str_free(fn->func_str);
  }
...
static void do_Static_assert(void) {
  ...
  if (tok == ')') {
    if (!c) tcc_error("_Static_assert fail");
    next();
  ...
  if (c == 0) tcc_error("%s", (char*)error_str.data);
  cstr_free(&error_str);
  skip(')');
static_assert_out:
  skip(';');
  ...
  while (1) {
    ...
    while (1) { /* iterate thru each declaration */
      ...
      if (ad.a.dllimport || ad.a.dllexport) {
        if (type.t & VT_STATIC) tcc_error("cannot have dll linkage with static");
...
#endif
      if (tok == '{') {
        if (l != VT_CONST) tcc_error("cannot use local functions");
        ...
        if (l == VT_CMP) {
          ...
          for (sym = func_vt.ref->next; sym; sym = sym->next)
            if ((sym->v & ~SYM_FIELD) == v) goto found;
        ...
        found:
          ...
          sym->a = ad.a;
          sym->f = ad.f;
          if (debug_modes) tcc_debug_typedef(tcc_state, sym);
          ...
          if (((type.t & VT_EXTERN) && (!has_init || l != VT_CONST)) ||
            ...
            if (ad.alias_target) {
              ...
              put_extern_sym2(sym, esym->st_shndx, esym->st_value, esym->st_size, 1);
            }
          } else {
