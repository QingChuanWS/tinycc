...
#if !defined(TCC_TARGET_I386) && !defined(TCC_TARGET_ARM) && !defined(TCC_TARGET_ARM64) && \
    !defined(TCC_TARGET_C67) && !defined(TCC_TARGET_X86_64) && !defined(TCC_TARGET_RISCV64)
...
#if defined TARGETOS_OpenBSD || defined TARGETOS_FreeBSD || defined TARGETOS_NetBSD || \
...
#define CONFIG_TCC_SYSINCLUDEPATHS \
  "{B}/include"                    \
  ":" ALSO_TRIPLET(CONFIG_SYSROOT  \
                   "/usr/local/include") ":" ALSO_TRIPLET(CONFIG_SYSROOT CONFIG_USR_INCLUDE)
#endif
#endif
...
struct SymAttr {
  unsigned short aligned : 5, /* alignment as log2+1 (0 == unspecified) */
      packed : 1, weak : 1, visibility : 2, dllexport : 1, nodecorate : 1, dllimport : 1,
      addrtaken : 1, xxxx : 3; /* not used */
};
...
static inline int toup(int c) { return (c >= 'a' && c <= 'z') ? c - 'a' + 'A' : c; }
...
ST_FUNC void relocate(TCCState* s1, ElfW_Rel* rel, int type, unsigned char* ptr, addr_t addr,
                      addr_t val);
...
static inline uint16_t read16le(unsigned char* p) { return p[0] | (uint16_t)p[1] << 8; }
...
static inline void add32le(unsigned char* p, int32_t x) { write32le(p, read32le(p) + x); }
...
static inline void add64le(unsigned char* p, int64_t x) { write64le(p, read64le(p) + x); }
...
#define tcc_warning_c(sw)                                                                      \
  TCC_SET_STATE((tcc_state->warn_num = offsetof(TCCState, sw) - offsetof(TCCState, warn_none), \
                 _tcc_warning))
